 ...
 * уезжает и в ответ админской ручки, и в факт контракта, поэтому второй
 * формы (живой объект, `Date` вместо epoch ms) не заводится.
 */
 ...
 *
 * Живого объекта наружу не уезжает: `ctx` и административный контроллер
...
export interface SubscriptionInfo {
  ...
  /** Транспорт ручки (`http`, `cli`, `bus`, …) */
  readonly transport: string;

  /** Паттерн ручки — то же значение, что видит дискавери */
  ...
  /** Момент открытия, epoch ms: то же значение уезжает в факт контракта */
...
/**
 * Форма `output` ручки как `kind` снимка.
 *
