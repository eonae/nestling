...
`errors:` endpoint, как в [главе 9](./09-auth.md).

Тот же способ работает в глубине вызовов: сервис, у которого нет канала
возврата к границе, бросает отказ, и хендлер его не перехватывает.
Компилятор бросок не видит, поэтому такой отказ обязан быть объявлен в
`errors:`: незадекларированный рантайм заменит на `InternalError` (500).
Канон гайда — `return`: возвращённый отказ виден в типе `Output`, и
список исходов хендлера описан целиком.
