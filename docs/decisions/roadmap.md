...
| 54 | `outbox` | Пакет `@nestling/outbox`: семейство `outboxed(Op)`, интерфейс `OutboxStore` с реализацией в памяти, relay ресурсом с `drain()`, секция конфига, факты `outbox.published` и `outbox.stuck`, политика `requiresTransaction`; глава 26 гайда | L | план — [ideas.md [2026-09-07]](./ideas.md) «Транзакционный outbox»; закрывает [deferred [2026-07-13]](./deferred.md) в части outbox'а |
...
| 54 | `outbox` | L | событие, отправленное соседу, не переживало падение процесса между коммитом и отправкой — дыра в главном обещании модульного монолита; второй мотив — замер границы ядра на задаче тяжелее реестра подписок |
