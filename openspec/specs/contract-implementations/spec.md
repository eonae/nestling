...
Реализация контракта — обычная endpoint-декларация на транспорте шины,
...
а subject остаётся именем контракта.
...
subject'ом, видом контракта и (для `event`) именем подписчика.

`input`, `output` и `errors` декларации SHALL браться из контракта и SHALL NOT
переобъявляться в словаре `implement`: интерфейс операции принадлежит
контракту, реализация добавляет только исполнение.
...
- **WHEN** приложение объявляет политику, требующую слой у каждой ручки, а
  реализация контракта собрана без этого слоя
...
создания декларации с указанием контракта и его вида. Имя подписчика SHALL
...
- **THEN** вызов бросает ошибку, называя контракт, вид `event` и требование
  указать `subscriber`

#### Scenario: Имя подписчика у запроса

- **WHEN** объявлено `implement(ChargeCard, { subscriber: 'billing', handle })`
  для контракта вида `request`
- **THEN** вызов бросает ошибку: у запроса ровно один владелец, и подписчиков
  у него не бывает

### Requirement: У `request` и `command` ровно один владелец

Две реализации одного контракта вида `request` или `command` в собранном
приложении SHALL быть ошибкой фазы ASSEMBLE. Текст SHALL называть контракт и
...
- **THEN** сборка падает на ASSEMBLE, называя контракт и оба модуля

#### Scenario: Событие без подписчиков

- **WHEN** приложение объявляет контракт-событие и эмиттер, но ни одной
  реализации
...
- **WHEN** второй `implement` того же контракта живёт в фиче, не попавшей в
  `select`
