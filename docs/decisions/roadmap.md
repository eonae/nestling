...
| 39 | `docs-layout-cleanup` | S | волна 1 уборки документации: генератор сайта переезжает в `scripts/site/`, вывод в `docs/.site/` вне git, правила ведения остаются одним экземпляром в `docs/README.md`, корневые README сокращаются, приложение В удаляется — [ideas.md [2026-09-05] «Структура документации»](./ideas.md), [d/11](../history/discussions/11-docs-structure-and-site.md) |
...
Уборка документации идёт пятью волнами, и `docs-layout-cleanup` — первая
из них. Следующие четыре заводятся отдельными change'ами: **волна 2** —
