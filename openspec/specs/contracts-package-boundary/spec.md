...
runtime-импортов, — чтобы принадлежность `.caller`/`.emitter` семействам
...
#### Scenario: Вызывающие стороны остаются токенами семейств

- **WHEN** операция, созданная из `@nestlingjs/operations`, реализована и её
  `.caller` инжектирован
- **THEN** токен распознаётся как токен семейства вызывающих сторон — так же,
  как до переезда
