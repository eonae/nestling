...
| 16 | `async-context` | M | **done** — архив появится шагом `/opsx:archive`; ридеры `Ctx(Var)` — семейство из 5, предикат `hasVar` — машинерия 28 |
