...
HTTP-специфика ответа выражается значением своего транспорта, а не полями
общего результата. Хендлер анонимной `httpEndpoint`-декларации получает
запрос в `meta.http` и возвращает `HttpResponse` — конверт с заголовками,
cookie или редиректом. Ядро знает только протокол конверта:
`TransportResponse` помечен именем транспорта, пайплайн разбирает его
`result` тем же кодом, что и обычный ответ, а метаданные читает лишь
транспорт с совпадающим именем. `httpEndpoint.implement` и `implement` ни
конверта, ни `meta.http` не принимают: хендлер операции остаётся
переносимым между транспортами, и это проверяет компилятор. Редирект
объявляется полем `redirect` декларации, поэтому попадает и в документ
OpenAPI.
...
нужен он только шагу `withClientIp`. Часов на запросе SHALL NOT быть:
