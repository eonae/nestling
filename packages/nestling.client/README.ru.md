...
Отказ клиент читает из документа RFC 9457
([design](../../docs/design/errors.md)): код отказа берётся из члена
`type` отбрасыванием префикса `urn:error:`, сообщение — из `detail`,
детали — из `details` по схеме определения. Тип без этого префикса,
отсутствующий тип и код, не объявленный в `errors:` операции, дают
`InternalError` с телом ответа в `cause`.
