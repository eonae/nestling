 ...
 * контракта и хранится на значении в виде карты `HttpBinding`. Карту читают
 ...
 * получать клиент, который импортирует контракт без серверного кода.
 *
 ...
 * карту читает и браузерный клиент. Методы WebDAV контракт не адресует;
...
export interface HttpBinding {
   ...
   * имя контракта нет. Имя читает генератор документации: из него
 ...
 *
 * Тип общий для `httpEndpoint` и секции `http` контракта.
 */
...
export interface ComputeHttpBindingOptions {
  ...
  /**
   * Имя контракта, которому принадлежит адрес. Задаёт только
   * `makeContract`.
   */
  contract?: string;

  /**
   * Как назвать владельца в тексте ошибки: `Contract '<имя>'` для
   * контракта. Если не задано, используются метод и путь.
   */
  where?: string;
}
 ...
 *
 * Вызывается при создании декларации (`httpEndpoint`) или контракта
 * (`makeContract`), а не при регистрации в приложении: карта нужна
