/**
 * Разбор ответа: успех, валидация по `output` и рематериализация отказа.
 *
...
/** Проводное тело отказа — то же, что собирает серверная граница */
 ...
 * ради экзотического кода новый элемент значило бы расширять контракт
...
/** Отказ «что-то пошло не так, и контракт этого не описывает» */
...
/**
 * Восстанавливает отказ по коду из `errors:` контракта.
 *
 * Статус берётся из **определения** (сервер мог ответить любым — контракт
 * знает лучше), `message` — с провода (он про конкретный случай), `details`
 * — с провода, но проверенные схемой определения. Не сошлись детали —
