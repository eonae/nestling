...
- [ ] 3.5 Занятые ключи: тип поля `pino` снимает их через `Omit`,
      конструктор проверяет их и вложенный `serializers.err` и даёт
      `TypeError` с именем ключа и заменой, текст английский
- [ ] 3.6 Три формы вызова на каждом из четырёх уровней и `child(bindings)`
      поверх `pino.child`
- [ ] 3.7 Барель: `pinoLogger` и `PinoLoggerOptions`, без `export *`

## 4. Тесты пакета

- [ ] 4.1 Формы вызова, уровни и привязки — перехватом
      `process.stderr.write`, как в `console.spec.ts`
- [ ] 4.2 Совпадение с штатным логгером: одна последовательность вызовов
      через оба логгера в `text`, строки сравниваются после подмены
      времени
- [ ] 4.3 Формат `json`: `time` строкой ISO, `level` меткой, `msg`, `err`
      объектом с `cause`, нет `pid` и `hostname`
- [ ] 4.4 Порог: `trace`, `fatal`, `silent` и умолчание `info`
- [ ] 4.5 Занятые ключи дают `TypeError`; `serializers` другого ключа и
      `redact` доходят до библиотеки
- [ ] 4.6 `boundary.spec.ts`: в поставляемом коде нет `@nestlingjs/app`,
      состав зависимостей — `@nestlingjs/logging` и peer `pino`

## 5. Документация

- [ ] 5.1 README пара нового пакета: шесть разделов в порядке, плашка со
...
- [ ] 5.2 README пара `@nestlingjs/logging`: три новых имени в «Экспортах»
      на обоих языках
- [ ] 5.3 `docs/README.md`: строка нового пакета в таблице «Инструменты и
      сателлиты», счёт каталогов «Двадцать три»
- [ ] 5.4 Глава 9, раздел «Свой логгер» (RU и EN): выдуманный
...
- [ ] 5.5 `design/composition.md`, раздел «Логгер» (RU и EN): итоговая
      форма опций сателлита, писатель в `stderr`, разница форматов `text`
      и `json`
- [ ] 5.6 Проверить `docs/glossary.md`, `docs/compatibility.md` и их
      английские пары: нужна ли строка о новом пакете
- [ ] 5.7 `node .claude/skills/docs-style/scripts/lint.mjs` по всем
      изменённым текстам — 0 запрещённых слов

## 6. Закрытие

- [ ] 6.1 `yarn verify` и `yarn docs:audit` зелёные
- [ ] 6.2 `roadmap.md`: строка 94 переходит в **done** с итогом
- [ ] 6.3 `ideas.md`: пометка «РЕАЛИЗОВАНО» записи [2026-09-13] дополняется
