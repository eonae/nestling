...
У pre-юнита три исхода. Первые два видны здесь: добавка к контексту
(`{ caller }`) и отказ (`Unauthorized()`). Третий — досрочный успех: юнит
возвращает `done()`, и endpoint завершается успехом, не доходя до
хендлера.

Досрочный успех объявляется тем же вторым аргументом:
`.pre(unit, { done: true })`. Декларация с таким слоем обязана быть без
`output` — значения досрочный успех не несёт, — и это проверяется при её
создании. Юнит, вернувший `done()` без объявления, роняет запрос ошибкой
с починкой в тексте.
