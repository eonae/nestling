...
| `check()` | 0–1 | структурный смок: граф строится, `@OnInit` не выполняется, ресурсы не захватываются; возвращает отчёт о составе (фичи, ручки по транспортам, транспорты) и бросает те же ошибки, что бросил бы `run()` на этих фазах |
