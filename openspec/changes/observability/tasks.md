...
- [ ] 9.1 Обновить `docs/design/pipeline.md`: переменная `Trace`, юнит
  `withTracing()`, запись метрик в ответной фазе
- [ ] 9.2 Обновить `docs/design/container.md`, раздел о логгере ядра:
  рядом появляется корень и семейство метрик
- [ ] 9.3 Обновить `docs/design/transports.md`: граница переноса трассы —
  шина и HTTP
- [ ] 9.4 Добавить в главу 9 гайда раздел о `withTracing()` и поле
  `traceId`, обновить дату в плашке «сверено с кодом»
- [ ] 9.5 Добавить в главу 20 раздел о сквозной трассе между процессами,
  обновить дату в плашке
- [ ] 9.6 Написать главу 22 о метриках на примере `split-nats`; добавить
  строку в таблицу части 2 и понятия в карту понятий `docs/guide/README.md`
- [ ] 9.7 Переименовать иллюстративный переключатель `Metrics` в
  `docs/design/composition.md`, `docs/conventions.md`, `docs/glossary.md` и
  `docs/guide/README.md`: имя занято типом ядра
- [ ] 9.8 Добавить в `docs/glossary.md` термины «трасса», «участок трассы»
  и «метрика»
- [ ] 9.9 Обновить README пакетов `nestling.app`, `nestling.client` и
  `nestling.testing`, включая плашки статуса и перечень экспортов
- [ ] 9.10 Прогнать `node .claude/skills/docs-style/scripts/lint.mjs` по
