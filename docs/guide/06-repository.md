...
Список подсказывает линтер. Правило `dependency-list` из
`@nestlingjs/eslint-plugin` выводит ожидаемые DI-токены из типов
параметров: `UsersRepository$` для интерфейса `UsersRepository` и
`Logger$.auto` для `Logger`, когда эти DI-токены видны в файле, `AppConfig`
для `Config<typeof AppConfig>`, сам класс для класса. Напишите `@Handler()`
с пустым списком, и `eslint --fix` заполнит его именами, которые файл уже
импортирует. Тип, которого правило не знает, оно принимает как написано,
поэтому уровень правила `warn`.
Гарантию даёт компилятор. Как подключить плагин, показывает
[глава 10](./10-auth.md).
