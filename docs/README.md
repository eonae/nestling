...
Документация собирается командой `yarn docs:build` в один файл
`docs/.site/index.html`. Источники сборки перечисляет
[`scripts/site/sections.mjs`](../scripts/site/sections.mjs): папки
`guide/` и `recipes/`, страницы `guarantees.md` и `from-nestjs.md`.
Генератор и тема лежат в
