...
не попадает. Как это работает — [глава 10](./10-auth.md).
...
[главы 9](./09-logging.md).

## `.ok`-юнит

В примере `.ok`-юнита нет. По README `@nestling/app` он вызывается
...
[глава 17](./17-select.md). Строка без объекта подходит, когда замыкание
