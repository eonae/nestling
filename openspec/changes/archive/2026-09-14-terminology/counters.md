...
перебазирована на него, и цифры «до» пересняты. Дальше `main` уходил
вперёд ещё дважды (`zod-inside`, `subscription-identity`), и ветка
перебазировалась на него же.

**Если ветка снова отстала.** Конфликты словарные, и чинятся они одним
рецептом: взять версию `main` (`git checkout --ours -- <файлы>`), прогнать
по ним ту же таблицу замен, потом вернуть на место четыре строки, которые
замена портит, — `| шаг | юнит, unit, middleware |` и
`| pre-шаг | pre-юнит, pre-unit |` в `docs/glossary.md`,
`| токен семейства | член семейства, членский токен |` там же и их
английские пары в `docs/en/glossary.md`. В этих строках старое имя и есть
содержание строки. После прохода переставить точку сверки в плашках глав
и рецептов на коммит переименования кода.
