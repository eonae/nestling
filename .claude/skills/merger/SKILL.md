...
| `UNLOCKED` | Сессия влитого worktree закрылась. Шаг «Уборка» из процедуры: убрать worktree и ветку |
...
5. **Уборка.** Сразу после проверки — и снова по событию `UNLOCKED`, когда
   сессия закроется позже. Worktree без `locked` и без живой сессии в `ListAgents`:
...
6. **Сессии change'а — «всё».** `SendMessage` той же сессии: change влит,
   проверки зелёные, работа закончена; пусть скажет пользователю одной
   строкой «change X влит в main и проверен — сессию можно закрывать» и
   больше ничего не делает. Промежуточное «слито, проверяю» этого не
