/**
 * Граница пакета проверяется тестом, а не обещанием.
 *
 * «Zero runtime deps» в README — конвенция; принцип **guarantee over
 * convention** требует, чтобы граф импортов физически не доходил до
 * серверного кода, и чтобы откат ломался громко и сразу. Обходится
 * замыкание собранного `dist/`: типовые импорты компилятор стёр, а
 * оставшееся — то, что действительно исполнится у потребителя.
 */
...
/**
 * Белый список замыкания.
 *
 * `@nestling/container/tokens` записан именно subpath'ом: барель пакета в
 * список не входит, и случайный откат на него — нарушение.
 */
const ALLOW = [
  '@common/misc',
