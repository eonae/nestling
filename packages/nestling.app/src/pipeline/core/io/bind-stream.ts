 ...
 * отложенные `.finally`-шаги. Объект-итератор финализирует запрос
 ...
 *
 * Именно закрытие итератора выполняет отложенные `.finally`-шаги, поэтому
...
export function withFinish<T>(
  ...
  return {
    ...
    async return(value?: unknown): Promise<IteratorResult<T>> {
      ...
      // Источник закрывается до финализации: `.finally`-шаги видят
