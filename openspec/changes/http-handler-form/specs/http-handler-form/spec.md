...
`method`, `url` (путь с query-строкой, как прислан клиентом), `headers`,
`ip` (адрес сокета, необязательный) и `receivedAt` (момент приёма
запроса). Транспорт SHALL класть значение в стартовый контекст запроса
под ключом `http`.

Поле `receivedAt` SHALL быть источником длительности для `httpAccessLog`:
юнит фазы `.finally` вызывается один раз за запрос, и другого per-request
якоря времени у него нет.
