 ...
 */
export type VisitCallback<T extends INode<T>> = (node: Readonly<T>) => void | Promise<void>;
...
export class DAG<T extends INode<T>> {
   ...
   */
  async traverse(callback: VisitCallback<T>, options: VisitOptions<T> = {}): Promise<void> {
    ...
    if (direction === 'topological') {
      await this.topologicalTraversal(callback, shouldVisit);
    } else {
      await this.reverseTopologicalTraversal(callback, shouldVisit);
    }
  }
  ...
  ensureAcyclic(): void {
    ...
    if (cycles.length > 0) {
      const cycleDescriptions = cycles.map((cycle, index) => 
  ...
  private async topologicalTraversal(
    ...
    while (queue.length > 0) {
      ...
      for (const dependent of dependents) {
        if (!shouldVisit(dependent)) continue;
  ...
  private async reverseTopologicalTraversal(
    ...
    while (queue.length > 0) {
      ...
      for (const dependency of node.dependencies) {
        if (!shouldVisit(dependency)) continue;
  ...
  */
  private getDependents(node: T): ReadonlyArray<T> {
    const dependents: T[] = [];
