...
Ядро считает запросы и вызовы само. Интерфейс `Metrics` даёт два пишущих
метода, корень `RootMetrics$` задаётся опцией `makeApp({ metrics })`, а
семейство `Metrics$(scope)` раздаёт его узлам графа — по образцу `Logger$`.
Счётчик и длительность на endpoint пишет рантайм пайплайна, на вызов порта —
узел порта: обе точки знают исход. Умолчание ничего не делает и ничего не
создаёт на горячем пути, поэтому приложение без метрик за них не платит.
Формата экспорта ядро не знает — числа уходят через стороннюю реализацию
корня.
