...
секций (как в рецепте [«Конфиг из файла и без
перезапуска»](./config-sources.md)), а `configKernel` вносит
...
`makeApp`, а фазу ему называет вызывающий код (рецепт [«Кто сейчас
...
Скрипт пишет его в файл, а `@nestlingjs/viz` рисует в браузере. Транспорт
