# 12. Оповещать соседей о случившемся
...
отличием: поле `subscriber` обязательно. Оно даёт подписке имя. Внутри
процесса паттерн endpoint'а складывается как `users.registered@quotas`,
и два подписчика одного события различаются именами. У брокера имя
становится именем группы получателей, поэтому его задаёт автор, а не
фреймворк.
...
хук шины. Если подписчиков нет, `emit` завершается сразу.
...
### Шаг 4. Команда с ключом идемпотентности
...
`idempotencyKey`. У события и запроса его нет, и обращение к нему у них
не компилируется.
...
- Поле `subscriber` обязательно у реализации события и запрещено у
  реализации запроса и команды: нарушение останавливает сборку.
- Два подписчика одного события с одним именем останавливают сборку.
- Событие без подписчиков собирается и работает: `emit` завершается
  сразу.
- Поле `idempotencyKey` есть только в `meta` команды. Тип `meta`
  выбирается по виду операции, и лишнее поле у события или запроса не
  компилируется.
- Команда всегда доставляется с ключом: переданным или сгенерированным.
- Политика `hasVar(IdempotencyKey)` останавливает сборку, если пайплайн
  реализации не объявляет переменную.
...
- `packages/examples.app-with-http/src/operations.ts` — событие и
  команда.
- `packages/examples.app-with-http/src/features/quotas/quotas.feature.ts`
  — подписчик события и реализация команды.
- `packages/examples.app-with-http/src/features/quotas/signup.journal.ts`
