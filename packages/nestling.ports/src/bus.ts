 ...
 * маршруты реализаций операций на их subject'ы, а через
...
export interface BusMessageMeta {
   ...
   *
   * Это часть конверта, а не payload: во вход операции они не попадают.
   */
...
export interface PublishOptions {
   ...
   *
   * Значение берётся из операции: долговечность — свойство операции,
...
export interface IMessageBus {
   ...
   *
   * Приложение с долговечными операциями на шине без этой возможности
   * всё равно стартует, а список таких операций печатается при старте.
   */
...
export class InProcessBus implements IMessageBus, ITransport {
  ...
  async publish(
    ...
    // нет. О таких операциях приложение печатает строку при старте
