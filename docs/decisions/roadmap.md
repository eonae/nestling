...
| 66 | `skill-setup` | скилл после первого внешнего прогона: спека собирает `app` из сниппетов через `assembleTest`, новые `references/setup.md` (tsconfig, скрипты, `tsx` для разработки, условие `testing`, конфиг `@nestlingjs/eslint-plugin`) и `references/http.md` (`redirect:`, `HttpResponse`, `Ok.created`), правило 6 в SKILL.md, строки «Where to look next» для шести пакетов, вариант `node:test` | S–M | план — [ideas.md [2026-09-12]](./ideas.md) «Скилл после первого внешнего прогона», [d/14](../history/discussions/14-first-external-run.md) пункты A1–A3, A5–A7, B8 |
