...
Пять правил ESLint для кода на Nestling: граница модуля по баррель-файлу,
форма условной примеси поля в объектный литерал, запрет глобалей процесса,
...
| `conditionalSpread` | правило `conditional-spread`: условная примесь поля в объектный литерал пишется связкой `&&` с явным сравнением. Тернарник с пустым литералом (`...(test ? obj : {})`) переписывает автофикс — но только когда итоговый тест булев по форме; иначе выбор сравнения остаётся автору. Примесь `...(x && obj)` с небулевым по форме `x` сообщает о ловушке нуля и пустой строки и предлагает `x !== undefined` suggestion'ом. Проверка полная по форме, уровень `error` |
| `noProcessGlobals` | правило `no-process-globals`: чтение `process.env` и `process.argv` в исходниках. Окружение доходит до приложения источником `env()`, командная строка — маркером `argv()`, и разрешена одна форма — `process.argv` непосредственным аргументом маркера. Именованный импорт `env` или `argv` из `node:process` — та же ошибка. Проверка полная по прямой записи, уровень `error` |
...
`conditional-spread` смотрит только на спред внутри объектного литерала:
в массиве та же замена ломает код, потому что спред массива требует
итерируемое. Булевость теста оно определяет формой выражения, а не типом,
поэтому идентификатор `ready` небулев даже при типе `boolean`.
`no-process-globals` не ловит обращение через промежуточную переменную
(`const p = process; p.env`): оно закрывает случайную запись, а не всякую
возможную. Законное чтение глушится директивой `eslint-disable` с
причиной в самом файле.
