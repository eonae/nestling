...
| 54 | `outbox` | Пакет `@nestling/outbox`: семейство `outboxed(Op)`, интерфейс `OutboxStore` с реализацией в памяти, relay ресурсом с `drain()`, секция конфига, факты `outbox.published` и `outbox.stuck`, политика `requiresTransaction`; глава 26 гайда | L | **done** — [архив](../../openspec/changes/archive/2026-09-07-outbox/), новая спека [`transactional-outbox`](../../openspec/specs/transactional-outbox/spec.md), [ideas.md [2026-09-07]](./ideas.md) «Транзакционный outbox»; закрывает [deferred [2026-07-13]](./deferred.md) в части outbox'а |
