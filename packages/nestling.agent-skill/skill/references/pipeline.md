...
A pipeline is what runs around a handler. It is built from steps by
...
|---|---|---|---|
| `.pre(step)` | before the handler | the context | add typed fields, or return a failure |
| `.ok(step)` | after a successful handler | the value and the context | replace the value |
| `.catch(step)` | after a failure | the failure and the context | replace it with another failure |
| `.finally(step)` | on every outcome | the outcome, the response, the context | observe only |

There is no `next()` and there is no wrapping. A `.pre` step that returns a
failure ends the request: no later step and no handler runs. A `.catch`
step can only exchange one failure for another — it cannot turn a failure
into a success.

A step is a function when it needs nothing from the container, and a class
marked `@Handler([…])` when it does. A step class is a provider like any
...
`compose(a, b)` stacks layers: the `.pre` steps of `a` run before those of
`b`, and the response-side steps run in reverse. Failures declared next to
a step — `makePipeline().pre(Authenticate, { errors: [Unauthorized] })` —
...
steps added, and the context variables of the request. A variable is read
...
is the built-in step that puts one there; `contextVar` declares your own.

## Policies

A policy is an invariant over the built graph. It is declared in
`makeApp({ policies })`, checked at the end of BUILD and before INIT,
