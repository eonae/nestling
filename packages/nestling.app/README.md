...
`makeApp({ config })` пакета `@nestling/app` вызывает `configKernel` сам, поэтому в декларации приложения про конфиг ничего писать не нужно, если хватает `process.env`. Собираете контейнер напрямую через `@nestling/container`, без `@nestling/app`, — зарегистрируйте `configKernel(bindings)` как обычный модуль.
