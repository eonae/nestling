...
сборку на фазе BUILD, а не даёт два экземпляра: у узла графа один
...
endpoint'ам, и они разделят один экземпляр: класс служит себе DI-токеном, и
...
Класс — не единственная форма. Поле `handler` принимает две:

| Форма | Запись | Когда |
|---|---|---|
| функция | `handler: async (input, meta) => …` | зависимостей нет, хендлер в две строки |
| класс | `handler: SomeHandler` | зависимости есть |

Зависимости из контейнера получает класс — то же правило, что у
провайдеров и у шагов пайплайна. Функция остаётся для хендлера, которому
получать нечего: служебный endpoint вроде `/ops/version` и первая глава
этого гайда. На поведение endpoint'а выбор формы не влияет.
...
глава подключает хранилище: [6. Откуда хендлер берёт
