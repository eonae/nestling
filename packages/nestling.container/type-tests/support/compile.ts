 ...
 *
 * Третья копия оснастки: первая живёт в `@nestlingjs/app`, вторая — в
 * `@nestlingjs/transport.http`. Копии уже расходятся — у `@nestlingjs/app`
 * есть часть про бюджет типов, здесь её нет. Вынос в общий пакет стоил бы
 * манифеста, конфигов и зависимости у трёх пакетов ради 150 строк тестовой
 * оснастки; порог пересматривается на четвёртом потребителе (design D5
 * change'а `feedback-polish`).
 *
...
/** `packages/nestling.container/type-tests` */
