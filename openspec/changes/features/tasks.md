...
- [ ] 11.7 Коммиты осмысленные, ветка запушена — *коммиты готовы (9 шт.);
      работа идёт в ветке `autorun/v1-all-waves` (все волны одним прогоном),
      а не в `change/features`. Push не прошёл по той же причине, что у
      предыдущих changes волны: `git@github.com` отвергает единственный
      доступный ключ (`Permission denied (publickey)`), HTTPS-путь тоже
      закрыт (`GH_TOKEN`/`GITHUB_TOKEN` не заданы, `gh` не установлен).
      Ветка ahead на 37 коммитов — требуется push с машины пользователя.
      Перепроверено 2026-07-31: `git push --dry-run` по-прежнему отвергается
      (`Permission denied (publickey)`), доступа не появилось*
