# 18. Разнести фичи по процессам, не меняя их код
...
как в главе [17](./17-select.md).

Роль процесса задаёт выбор фич, который `main.ts` читает из
`APP_FEATURES` до сборки, как в главе [17](./17-select.md).
...
Имя подписчика из главы [14](./14-events.md) здесь получает вторую
...
сломать соседний сервис: [19. Не сломать соседей при изменении
операции](./19-compatibility.md).
