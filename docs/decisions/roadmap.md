...
| 41 | `packaging-cleanup` | два дефекта упаковки: `tsconfig.build.json` не исключал `*.type-test.ts` и `__fixtures__`, и они попадали в `dist` вместе с импортом двух `devDependencies`; манифесты семи пакетов расходились с фактическими импортами | S | план — [ideas.md [2026-09-05]](./ideas.md) «Раскладка монорепы» |
...
| 41 | `packaging-cleanup` | S | опубликованный `@nestling/testing` нёс в `dist` файлы проверки типов и фикстуры, а один из них импортировал `@nestling/transport.http` и `zod` из `devDependencies`: у установившего их нет; заодно чинятся семь расхождений «объявлено ≠ импортируется» |
