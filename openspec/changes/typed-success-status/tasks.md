...
- [ ] 9.2 Новая запись `deferred.md`: потоковая форма веткой развилки —
      что придумано (настройки потокового ответа переезжают на ветку:
      реконнект SSE, поле `sse`, item-цепочка, capability-проверка),
      почему отложено, триггер возврата
- [ ] 9.3 Открытый вопрос дизайна (политика `check()` на недостижимую ветку
      развилки) записать туда же отдельной записью
- [ ] 9.4 После `/opsx:archive`: строка 84 в `docs/decisions/roadmap.md` —
