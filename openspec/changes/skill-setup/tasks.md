...
- [x] 8.6 Затронутые `examples/*` мигрированы, главы гайда пересверены с
  ...
  > **Вывод.** Затронутых нет. `git diff main...` трогает
  > `packages/nestling.agent-skill/`, `docs/decisions/`, `openspec/changes/`
  > и `yarn.lock`. Публичное API пакетов не менялось ни в одном файле, поэтому
  > ни один пример и ни одна глава гайда не расходятся с кодом: плашки
  > «сверено с кодом» остаются с прежними датами.
- [x] 8.7 Коммиты осмысленные, ветка `change/skill-setup` запушена
