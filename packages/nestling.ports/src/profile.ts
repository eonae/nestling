 ...
 *
 * Профиль — **конверт вызова**, а не ambient-состояние: он приезжает
 * словарём `meta`, едет через транспорт относительным timeout'ом и на
 * принимающей стороне доступен двумя каналами. Каналы разноуровневые и
 * потому не дублируют друг друга:
 *
 * - **транспортные атрибуты** (`ctx.raw.attributes`) — провод: их кладут
 *   оба вызывателя и шина, безусловно, рядом с `subject`;
 * - **ambient-переменные** ({@link Deadline}, {@link IdempotencyKey}) —
 *   проекция для кода произвольной глубины, включаемая штатным pre-юнитом
 *   и проверяемая политикой `everyEndpoint(…).hasVar(…)` на сборке.
 */
...
/**
 * Абсолютный момент из относительного бюджета — сахар call-site.
 *
 * `meta.deadline` принимает только `Date`: `500` одинаково правдоподобно
 * читается и как epoch-миллисекунды, и как «через 500 мс», а разница между
 * этими прочтениями — разница между мгновенным отказом и полусекундным
 * бюджетом.
 *
 ...
 * получателя**.
 *
 * Ровно эта пара превращений и есть «пересчёт остатка на каждом hop»: по
 * проводу едет длительность, внутри процесса живёт момент.
 */
...
export interface CallBudget {
   ...
   * Уезжает в контекст обработчика, поэтому кооперативная реализация видит
   * исчерпание бюджета своим `ctx.signal`.
   */
 ...
 * приехавшее вовсе без ключа (не с порта, а с произвольного транспорта),
