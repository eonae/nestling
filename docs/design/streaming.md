...
### Потоковая форма объявляется единственным исходом

`stream(...)` и `events(...)` стоят в слоте `output` одни: веткой развилки
`outputs({ … })` потоковая форма не объявляется
([endpoints.md §5](./endpoints.md)).

Причина в том, что настройки потокового ответа читаются до вызова
хендлера, когда исход ещё не известен. Транспорт кладёт `last-event-id` в
стартовый контекст на разборе запроса, секция `sse` объявляется при
`events(...)`, выходная item-цепочка навешивается по форме декларации, а
capability-проверка сверяет с транспортом одну форму. Endpoint, отвечающий
то потоком, то объектом, описывается двумя адресами.

### Момент финализации

У потокового `output` исход известен только после доставки, поэтому
`.finally`-шаги выполняются после завершения отдачи потока: когда поток
...
Число прочитанных элементов на финализацию не влияет. Закрытие итератора
до первого элемента выполняет `.finally` так же, как закрытие на середине
потока, и доводится до источника, который вернул хендлер: подписка,
занявшая место в момент вызова, снимается. Сколько элементов ушло
клиенту, наблюдатель читает из `ctx.summary.itemsOut`.
...
([errors.md](./errors.md)). `.catch`-шаги посреди потока не вызываются:
...
- `.through(src => wrapped)` принимает произвольную обёртку итератора: на
  входе с изменением типа, на выходе `T → T`. Обёртка обязана занимать
  ресурсы в теле итерации, а не при создании: закрытие потока, ни разу не
  прочитанного, идёт мимо шагов цепочки — прямо к источнику хендлера.
...
шагу, не только `.finally`. Элементы считает рантайм цепочек: `itemsIn`
...
`.pre`/`.finally`, класс-форме шага, `AbortSignal`, DI, `Topic` и
...
Гайд: рецепт [«Кто сейчас подключён и как его отключить»](../recipes/ops.md).
