...
У сервиса есть лента по SSE из [главы 15](./15-live-feed.md), и клиенты
...
[главы 13](./13-features.md). Класс-юниты слоя подписок регистрирует
...
сквозной слой из [главы 9](./09-logging.md).
...
[главе 10](./10-auth.md). В примере такой политики нет: слой подключён на
...
[главы 14](./14-events.md). Фича `ops` подписана на оба через
...
split-развёртывании из [главы 18](./18-split.md) один процесс собирает
...
снапшот совместимости из [главы 19](./19-compatibility.md).
...
[главе 10](./10-auth.md): политика «у каждого HTTP-endpoint'а есть слой
...
Вклад в отчёт — обычный провайдер члена семейства `HealthCheck$` в том
модуле, которому он принадлежит ([глава 22](./22-token-families.md)); у
...
топологию и выбирается явно, как показывает [глава 17](./17-select.md).
...
без приложения, [глава 25](./25-standalone.md).
