...
Голое значение из хендлера отвечает `200`. `Ok.created(value)` отвечает
`201`, `Ok.noContent()` отвечает `204` без тела, `Ok.accepted(value)`
отвечает `202`. Заголовков `Ok` не несёт: их задаёт HTTP-форма ответа
`HttpResponse.of(value, { headers })`. Статус успеха для документа OpenAPI
