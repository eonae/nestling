...
| 56 | `cli-input-prompt` | политика сбора недостающего входа в CLI: `missing: 'prompt'` достраивает payload из схемы `input` вопросами в терминале | M | **done** — [архив](../../openspec/changes/archive/2026-09-12-cli-input-prompt/), [ideas.md [2026-07-13]](./ideas.md) «Контракт первичен», п. 7 |
...
| 56 | `cli-input-prompt` | M | **done** — [архив](../../openspec/changes/archive/2026-09-12-cli-input-prompt/); обещание, вынесенное из scope change'а 24, жило комментарием в коде транспорта, а не строкой плана; попутно переименован пакет-конвертер — `@nestlingjs/openapi.zod` стал `@nestlingjs/schema.zod` |
...
| 59 | `docs-guide-recipes` | M, breaking | **done** — [архив](../../openspec/changes/archive/2026-09-12-docs-guide-recipes/); гайд вёл по пути и одновременно служил справочником, а общая нумерация не давала вставить главу в середину: главу о базе дописали в конец, и шесть глав пути ссылались на неё вперёд — самая дальняя через девятнадцать глав |
| 60 | `docs-site-sections` | M–L | **done** — [архив](../../openspec/changes/archive/2026-09-12-docs-site-sections/); сайт собирался одним файлом без разделов, поиска и метаданных — оглавления глубже глав у читателя не было |
| 61 | `docs-publish` | S–M | документация собирается локально и никуда не публикуется |
| 62 | `barrels-by-name` | S, breaking | **done** — [архив](../../openspec/changes/archive/2026-09-12-barrels-by-name/); правило про поимённый барель действовало на все пакеты, а выполняли его два из восемнадцати: в остальных барель состоял из `export *`, и публичным было всё, что экспортирует любой внутренний модуль |
...
| 70 | `eslint-deps-sync` | M | **done** — [архив](../../openspec/changes/archive/2026-09-12-eslint-deps-sync/); список зависимостей повторяет типы конструктора в каждом файле; компилятор ловит расхождение, а чинит его человек |
