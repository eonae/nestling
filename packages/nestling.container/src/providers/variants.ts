 ...
 *
 * Рецепт возвращает обычное определение провайдера для запрошенного токена семейства;
 * билдер проверяет, что его `provide` совпадает с токеном семейства.
 *
 * @template T - Тип значения каждого DI-токена
 * @template Params - Параметры токена семейства
 */
 ...
 * (массив или фабрика). В `build()` контейнер собирает всех токенов семейства,
 ...
 * @param recipe - Возвращает определение провайдера для параметра токена семейства
 ...
 * и карта операций считаются на фазе BUILD, когда узлов ещё нет.
 *
