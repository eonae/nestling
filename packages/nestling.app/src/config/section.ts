...
/**
 * Имя токена семейства проекции reloadable-секции, занятое подпиской.
 *
 ...
 *
 * DI-токен — сам токен семейства `ConfigSection`, на который дописан `.keys`.
 * Инжект DI-токена и упоминание токена семейства — одно и то же ребро графа, потому что
...
>(
  ...
  // DI-токен секции и токен семейства — одно значение: рецепт семейства
