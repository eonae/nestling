...
export interface IListener {
  ...
  /**
   * Базовый адрес сервера (`http://host:port`) — необязательная способность.
   *
   * Ядро знает роль, а не протокол: строка адреса — знание схемы, которого
   * у `IListener` нет. Листенер без сетевого адреса (будущие CLI/bus-роли)
   * её не реализует; `testApp.baseUrl(name?)` резолвит листенер и явно
   * отказывает, если способности нет.
   */
  baseUrl?(): string;
}
