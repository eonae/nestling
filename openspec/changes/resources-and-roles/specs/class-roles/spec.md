...
| хендлер | `@Handler([deps])` | конструктор, метод `handle` | слот `handler:` |
...
#### Scenario: Хендлер в `providers:`

- **WHEN** класс под `@Handler([UserService])` перечислен в `providers:`
  модуля
- **THEN** сборка падает на фазе ASSEMBLE ошибкой, называющей класс,
  `providers:` и `@Component`
