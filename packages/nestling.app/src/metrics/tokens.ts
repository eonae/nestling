 ...
 *
 * Замена значения под `RootMetrics$` меняет все токены семейства `Metrics$`: рецепт
 * семейства оборачивает корень, добавляя атрибут `scope`, и потребители
 * токенов семейства замены не видят.
 */
 ...
 * записи атрибут `scope: 'users'`. `Metrics$.auto` даёт токен семейства по имени
 * класса-потребителя.
 *
 * @example
 * ```typescript
