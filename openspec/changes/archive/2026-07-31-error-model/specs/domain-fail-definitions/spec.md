...
`INTERNAL_ERROR`) и `ValidationFailed` (код `VALIDATION_FAILED`, статус
`BAD_REQUEST`). Эти коды SHALL считаться частью множества допустимых
ответов **любой** ручки без объявления в `errors:`.

Набор kernel-кодов SHALL быть закрытым: публичного способа пометить
пользовательский код встроенным SHALL NOT существовать.
