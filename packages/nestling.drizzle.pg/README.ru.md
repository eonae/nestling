...
`drizzle-orm`, `pg` и `zod` — peer-зависимости: версию драйвера и версию
валидатора выбирает приложение, и второй копии ни того, ни другого в
процессе быть не должно. Адаптеры
...
Слой транзакции не применяется к потоковому ответу: шаг `.ok`
