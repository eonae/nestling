...
> «Пакет тестирования (`@nestlingjs/testing`)» [2026-07-10] — `check()`.
...
flowchart TD
    ...
    P1["1 · BUILD"] -->|"проверка графа · discover · bind ports"| P2
...
endpoint'ы; глобального реестра нет. Тот же счёт доступен и отдельно:
`app.discover(args?)` разбирает аргумент, раскрывает ветки, разрешает
выбор фич и проходит discovery — и на этом останавливается, не строя
графа и не поднимая источников конфига. Это публичный вход фазы 0 и вход
генератора документа ([schemas.md §2.1](./schemas.md)); ошибки графа
остаются за `check()`. Модули этих единиц регистрируются в контейнере. Секции конфига считаются из снимка и валидируются;
...
хендлера и классы шагов пайплайна (`endpoint.resolve(resolver)`). Затем для каждого транспорта собирается
...
`run()` ставит обработчики `SIGTERM` и `SIGINT`, переводящие приложение в
SHUTDOWN, и снимает их по завершении `close()`. Приложение внутри чужого
процесса ставит `run({ signals: false })`: за остановку процесса отвечает
хозяин, а подписка на `SIGINT` отменила бы его завершение по Ctrl+C.
Остальные фазы идут одинаково при любом значении опции.
...
BUILD — запись «Жизненный цикл» в журнале.

## 2. `makeApp` и `build`: composition root
...
export const app = makeApp({
  ...
  transports?,  // L0+: объявления экземпляров транспортов (§4)
  intercom?,    // L4: имя транспорта, переносящего операции
  logging?,     // корневой логгер и поля корреляции: { logger, fields } (§6)
  policies?,    // инварианты на собранном графе; проверяются в конце
                //   фазы 1 BUILD — в run(), check() и buildTest
                //   (pipeline.md §7)
...
// run({ config }) задаёт источники конфига (config.md §3); без опции — умолчание
...
Источники конфига декларации не принадлежат: их список принимает `run()`
([config.md §3](./config.md)). Поле `logging:` задаёт корневой логгер и
поля корреляции (§6). Логгер — готовое значение: корень существует раньше
графа и зависеть от его узлов не может, зато записи всех фаз, включая
предупреждения сборки, уходят в него ([container.md](./container.md),
«Логгер»). Подмена узлов графа
(`overrides`) есть только у тестового корня `buildTest`
...
`app.build(args?)` синхронна и ничего не читает; она возвращает
`BuiltApp`, фазы выполняет `run()`. Тип аргумента выводится из
декларации: `{ features?, includeDeps?, ...значения переключателей из switches: }`.
Точка входа передаёт аргументы командной строки маркером
`argv(process.argv)`: сборка разбирает их по той же схеме. `--features`
принимает `all` или список имён, `--include-deps` — флаг, каждый
переключатель — флаг со своим именем и значениями из `makeSwitch`;
`--help` печатает схему. Разбор строгий: неизвестный флаг, неизвестная
фича или чужое значение переключателя — отказ до фазы 0. Ядро
`process.argv` само не читает.

Маркер владеет процессом: вход, получивший его, завершает процесс сам.
`--help` печатает схему в `stdout` и завершает процесс кодом `0`. Отказ
печатается в `stderr` и завершает процесс кодом `1`. В тексте отказа
стоит сообщение с цепочкой причин, стека в нём нет. Правило действует у
всех трёх входов, принимающих маркер, и покрывает отказ любой фазы, а не
только отказ разбора. Объектная форма аргумента процессом не владеет:
`run()` отклоняет промис, `discover()` и `check()` бросают.

Декларация и собранное приложение предоставляют четыре метода:

| Метод | Где | Фазы | Что делает |
|---|---|---|---|
| `run(options?)` | `BuiltApp` | 0–5 | доводит приложение до RUN и остаётся там; ставит обработчики сигналов. `options.config` — привязки источников вместо умолчания ([config.md §3](./config.md)) |
| `check(args?, options?)` | `App` | 0–1 | структурная проверка: граф проверяется, экземпляры не создаются, ресурсы не захватываются; проверяет `policies:`; возвращает отчёт о составе (фичи, переключатели, endpoint'ы по транспортам с причинами `detached`, транспорты, карта операций) и бросает те же ошибки, что бросил бы `run()` на этих фазах. `options.config` — привязки источников вместо умолчания, как у `run()` |
| `discover(args?)` | `App` | 0 декларации | состав по аргументу сборки значением: фичи, переключатели, endpoint'ы по транспортам; ни источников, ни графа. Вход генератора OpenAPI ([schemas.md §2.1](./schemas.md)) |
| `close()` | `BuiltApp` | 6 | SHUTDOWN в строгом обратном порядке; идемпотентен |
...
не влияет на последующий `build()` той же декларации. Поэтому им
...
Сборка падает на BUILD в четырёх случаях: имя неизвестно (ошибка
...
Обе топологии на одном коде — `examples/modular-app`: один корень,
...
Сервер — ресурс `server({ name? })`, который держит сокет. В `transports:`
он не перечисляется: сборка собирает серверы из деклараций транспортов по
ссылке `server` и разворачивает их как узлы, один узел на ссылку. Сервер,
на который не ссылается ни один транспорт, не создаётся. Транспорт без
`server` объявляет собственный сервер с тем же именем. Несколько
транспортов на одном сокете ссылаются на одну переменную:

```typescript
transports: [http()],                                  // сервер по умолчанию: HTTP_PORT, HTTP_HOST

const api = server();                                  // тот же сервер по умолчанию, объявленный явно
transports: [http({ server: api }), mcp({ server: api })],

const admin = server({ name: 'admin' });               // HTTP_ADMIN_PORT, HTTP_ADMIN_HOST
```

Экземпляры обоих видов собранное приложение отдаёт по имени: серверы —
картой `app.servers`, транспорты — картой `app.transports`. Обе карты
только на чтение и пусты до фазы INIT. Через них берут у узла значение,
которого нет у объявления: фактический адрес у сервера при `HTTP_PORT=0`
и обработчик запроса у адаптера
([transports.md §4.3](./transports.md)).

Декларация ссылается на транспорт DI-токеном; если экземпляра нет в графе,
сборка падает на BUILD. Порт и адреса сервер и транспорт читают из
...
| форматирование и отправка ошибок | `.catch`- и `.finally`-шаги того же слоя ([errors.md](./errors.md)) |
| обогащение контекста | `.pre`-шаг слоя; чтение из глубины — ридеры асинхронного контекста |
...
`@nestlingjs/openapi` — инфраструктура и объявляет служебный endpoint с
документом.
...
сборки в корне. Снаружи графа то же значение отдаёт `app.discover(args?)`;
другого входа в discovery нет — аргумент сборки есть только у декларации,
а без него состав документа расходится с составом процесса. Значение read-only: списки заморожены, мутаторы карты
...
Забытый слой обнаруживается на BUILD с именем endpoint'а. Осознанное
...
обычным провайдером токена семейства: `classProvider(HealthCheck$('db'), DbHealthCheck)`.
...
Узел называет каждый вклад поимённо: имя проверки живёт в токене семейства,
а не в самом вкладе. Перечень токенов семейства и перечень ресурсов с `health` даёт
...
`RootLogger$` — DI-токен корневого логгера с интерфейсом `Logger` из
`@nestlingjs/logging` ([container.md](./container.md), там же интерфейс).
Корень задаёт опция `logging: { logger?, fields? }`. `logger` — готовое
значение, по умолчанию логгер ядра. `fields` — переменные контекста,
значения которых попадают в каждую запись, по умолчанию
`[RequestId, logField(Trace, 'traceId', (t) => t.traceId)]`. Сборка оборачивает переданный логгер декоратором
корреляции: он читает объявленные переменные из ambient-контекста и
добавляет их полями, поэтому любая реализация получает `requestId` и
`traceId`, не зная внутренностей ядра. Плагин объявляет свои поля в
`makePlugin({ logFields })`, и сборка собирает их вместе с провайдерами;
`logField(Var, 'name')` задаёт имя поля, иначе имя переменной. Два поля с
одним именем — ошибка сборки. Отсутствующее значение поле не ставит.
Статические поля процесса (`node`, `version`) добавляются через
`logger.child({...})` при создании. Уровень и формат логгера ядра задаёт
kernel-секция `nestlingLog`: `NESTLING_LOG_LEVEL` (`debug` | `info` |
`warn` | `error` | `silent`, по умолчанию `info`) и `NESTLING_LOG_FORMAT`
(`text` | `json`, по умолчанию `text`). Порог `silent` отсекает все четыре
уровня; тестовый прогон поднимается с ним ([testing.md](./testing.md)).
Записи уходят в `stderr`: у CLI-транспорта `stdout` занят результатом
команды.

`Logger$(scope)` — семейство с рецептом `root.child({ scope })`.
`Logger$.auto` даёт токен семейства по имени потребителя. Замена корня меняет все
токены семейства, потребители этого не видят. Ядро пишет только через
...
Отдельных хуков вывода у ядра нет: `console` ядро не трогает, а в поток
пишет только штатный логгер из `@nestlingjs/logging`. Предупреждения читалки конфига копятся до появления логгера и
