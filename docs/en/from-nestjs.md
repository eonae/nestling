...
|---|---|---|---|
| `NestFactory.create(AppModule)` and `app.listen()` | `makeApp({ features, transports }).assemble().run()` | `run()` takes the application through the phases and sets up the shutdown on `SIGTERM` itself | [1](./guide/01-first-service.md) |
...
| `Scope.TRANSIENT` with `INQUIRER` | `Family.auto` | a family member named after the consumer is created at assembly, not on every injection | [recipe](./recipes/token-families.md) |
...
| `Middleware` | a `.pre` unit | the unit extends the context with typed fields and does not call `next()` | [9](./guide/09-logging.md) |
| `Guard` | a `.pre` unit that returns a failure | the failure is declared in the `errors:` of the endpoint; the handler is not called | [10](./guide/10-auth.md) |
| `Interceptor` | `.pre`, `.ok`, `.finally` units | three separate phases instead of a wrapper around the call | [9](./guide/09-logging.md) |
| `ExceptionFilter` | a `.catch` unit | replaces one failure with another; a failure cannot be turned into a success | [recipe](./recipes/alternatives.md) |
...
| NestJS | Nestling | What differs | Chapter |
|---|---|---|---|
| injecting the service of another module | the `makeRequest` operation and `Operation.caller` | features are linked only by operations; a direct edge between features is an assembly error | [14](./guide/14-features.md) |
...
| a hand-written HTTP client | `makeClient(operations, config)` | the client is assembled from the same operations and restores failures by code | [13](./guide/13-openapi-and-client.md) |
...
| `Test.createTestingModule()` with `overrideProvider()` | `assembleTest(app, { overrides })` | the test assembles the same application through the same phases; the socket is not opened | [8](./guide/08-testing.md) |
...
- `next()` in request handling: pipeline units do not wrap each other.
