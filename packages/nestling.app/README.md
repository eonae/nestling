...
| 6 SHUTDOWN | `abort(signal)`, затем `close()` транспортов в обратном порядке, затем `container.destroy()`, затем `close()` источников конфига |
...
| `bootstrapConfig(bindings?, options?)` | фаза 0: поднимает источники и снимает снимок; возвращает читалку |
...
| `ConfigKernelOptions`, `ConfigWarn` | опции `bootstrapConfig` и канал предупреждений |
...
Интерфейс `ITransport` (`capabilities`, `serve(dispatch, signal)`,
`close?()`) и значение `Dispatch`, через которое корень передаёт
...
а не договорённость. Тип объявлен в слое пайплайна (набор форм —
