...
| `nestling.drizzle.pg` | 3 | `TS9007`×2, `TS9010`×1 (два исключены) |
...
`inboxTable()` и `outboxTable()` в `nestling.drizzle.pg` отдают наружу
объявление таблицы drizzle: 60 и 230 строк метаданных каждой колонки. Тип
принадлежит drizzle и меняется с её версией, поэтому пакет исключён из
правила `isolatedDeclarations` — с причиной в своём `tsconfig.json`. Его
декларации по-прежнему выводит `tsc` по всей программе: 2.3 секунды
процессорного времени из примерно 330 у всего прогона.

Тот же пакет исключён и из строгого прохода, и по той же причине — его
красит чужой код.
