...
последняя попытка доставки дойдёт до хендлера второй раз. Умолчание — семь
суток, и оно с запасом перекрывает окно дедупликации потока, которое
задаёт транспорт NATS (5 минут по умолчанию).
...
Окно дедупликации потока пакет не заменяет и заменяться им не может.
Брокер снимает быстрые повторы публикации: relay, вернувшийся к записи
внутри окна, второй копии в поток не положит. Повторы доставки и повтор,
пришедший позже окна, остаются — у транспорта нет транзакции приложения,
поэтому отметка и бизнес-изменение коммитятся вместе только здесь. Слой
приёма обязателен при любом окне.
