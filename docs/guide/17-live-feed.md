...
> Гайд по текущему API; сверено с кодом `c05b4909`.
...
`@nestlingjs/operations`: источник с любым числом подписчиков. `push` не ждёт
...
подписки свой буфер, поэтому медленный клиент не тормозит остальных.
...
задаёт поведение при переполнении буфера подписчика. При `drop-oldest`
(значение по умолчанию) самое старое событие отстающего подписчика
...
`stream(T)` из [главы 12](./12-files-and-streams.md):
...
Транспорт отдаёт элементы кадрами SSE. Он закрывает итератор при
отключении клиента и при остановке приложения: подписка снимается с
темы, буфер освобождается. Секция `sse:` задаёт поля кадра: `id`
попадает в строку `id:`, `event` в строку `event:`. Клиент, который
переподключился, присылает заголовок `Last-Event-ID`, и транспорт кладёт
его в `meta.lastEventId` уже типизированным полем, поэтому заголовок не
нужно читать из сырого запроса. Откуда продолжить, решает хендлер.

Слой `tracked` из пакета `@nestlingjs/subscriptions` регистрирует подписку
...
Для потокового ответа юниты `.finally` слоя `observability` выполняются
...
и уход клиента до первого события: `.finally` выполняется, а
`ctx.summary.itemsOut` остаётся нулём. Отказ посреди потока приходит
клиенту именованным событием `error`, и соединение закрывается. Имя
`error` зарезервировано: прикладное событие с таким именем отвергается
...
В app-тесте `testApp.call(ActivityStream)` возвращает ответ, у которого
`value` является `AsyncIterableIterator`: тест читает события через
`next()` без транспорта. Так устроены тесты реестра подписок в
`app.spec.ts`.

Фича `users` зависит от рассылки, но тест фичи не должен поднимать
соседа:
[18. Тестировать фичу без соседей](./18-testing-features.md).
