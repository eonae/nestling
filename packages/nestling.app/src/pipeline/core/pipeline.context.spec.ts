 ...
 * из шага, хендлера, ответной фазы и тела потока. Ридер здесь
...
describe('проекция обновляется после каждого .pre-шага', () => {
  it('шаг видит поля предыдущих шагов и не видит своих последователей', async () => {
...
describe('ответная фаза под тем же scope', () => {
  ...
  it('после отказа .pre-шага peek() даёт undefined, а get() бросает', async () => {
