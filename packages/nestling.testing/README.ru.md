...
`checkTopologies` видит недостижимые вызовы: операция, которую в выбранной
топологии никто не реализует, а интерком не назначен, роняет сборку на фазе
`1 BUILD` — и матрица называет такую топологию среди несобравшихся.
Застабанное имя (`stubs:`) проверки не проходит вовсе: явный провайдер
опережает рецепт вызывателя.
