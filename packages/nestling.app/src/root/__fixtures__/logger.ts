 ...
 *
 * Поле `logging.logger` корня — единственный способ заменить корень,
 * поэтому все записи ядра и приложения, включая записи фаз 0 и 1, попадают
 * в `entries`.
 */
...
/** Шпион логгера: значение для поля `logging.logger` корня */
export type LoggerProbe = SpyLogger;

/** Создаёт шпион для `logging: { logger }` корня */
