...
export type PluginOptions = BundleOptionsBase &
  BundleComposition & {
     ...
     *
     * Только плагины: параметризованную зависимость выражают DI-токеном, а не
    ...
    if (dependency?.role !== 'plugin') {
      throw new TypeError(
        `makePlugin({ name: '${name}' }): 'dependsOn' accepts plugins only. ` +
          ...
          `parameterized plugin is required by its DI token, not by this field.`,
      );
    }
  }
...
/**
 * DI-токены, которые единица запрашивает у контейнера: зависимости её
 * классов-хендлеров и её провайдеров.
 *
 * Вызыватель операции — обычный DI-токен, поэтому инжектировать его может и
