/**
 * Слой `tracked` — два шага и ничего больше.
 *
 * Оба шага — класс-формы: функциональная форма зависимостей не имеет, а
 * реестр это singleton графа. Класс резолвится контейнером на WIRE, поэтому
 * endpoint со слоем, но без модуля `makeSubscriptions({ … })` отказывает на
 * BUILD, а не на первом запросе.
 */
