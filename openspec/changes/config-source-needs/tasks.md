...
- [ ] 1.1 Параметризовать `ConfigSource<Needs = void>` в
  `packages/nestling.app/src/config/source.ts`: поле
  `needs?: ConfigSectionToken<Needs>` и `init?(values: Needs)`
- [ ] 1.2 Сделать `bind()` дженериком по `Needs`; хранить в `Binding`
  источник как `ConfigSource<unknown>`, чтобы список привязок оставался
  однородным
- [ ] 1.3 Проверить типами, что источник без `needs` пишется как раньше:
  `*.type-test.ts` на `env()`, `dotenv(path)` и источник с `needs` чужой
  формы (ожидаемая ошибка компиляции)

## 2. Очерёдность подъёма

- [ ] 2.1 Считать граф зависимостей привязок в `reader.ts`: ребро «`A`
  после `B`», если таргет `B` покрывает ключ секции `A.needs` и `B` — не
  сама `A`
- [ ] 2.2 Стабильный топологический порядок: не связанные зависимостью
  привязки сохраняют порядок списка
- [ ] 2.3 Отказ по циклу до первого `init()` — новый `ConfigSourceCycleError`
  с цепочкой имён источников; `optional` его не проглатывает
- [ ] 2.4 Писать порядок подъёма в логгер ядра уровнем `debug`
- [ ] 2.5 Спеки: порядок при пустых `needs`, Vault выше `.env` в списке и
  позже него по подъёму, цикл из двух и из трёх источников

## 3. Проекция секции `needs`

- [ ] 3.1 Перед `init()` зависимого источника находить декларацию секции по
  префиксу `needs.keys`; отсутствие в реестре — отказ с префиксом и именем
  источника
- [ ] 3.2 Отказывать, если секция объявлена `makeConfig.reloadable`
- [ ] 3.3 Читать ключи секции только из привязок, поднявшихся раньше, и
  фиксировать значения в снимке; финальный проход по объявленным ключам не
  перечитывает то, что уже лежит в снимке
- [ ] 3.4 Исключить привязку из разрешения ключей её собственной секции
  `needs` — в единственной точке поиска значения (`#lookup`)
- [ ] 3.5 Проецировать и валидировать секцию тем же кодом, что и граф;
  помечать декларацию потреблённой
- [ ] 3.6 Отказ проекции — новый `ConfigSourceNeedsError`: источник,
  секция, недостающие ключи, перечень поднятых источников, исходная ошибка
  в `cause`; секретные значения в текст не попадают
- [ ] 3.7 `optional: true` проглатывает отказ проекции так же, как отказ
  `init()`
- [ ] 3.8 Спеки: значения приходят в `init(values)`; источник, поднятый
