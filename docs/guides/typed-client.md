...
У контракта три потребителя. Два из них живут внутри приложения: реализация
(`implement` или контракт-форма `httpEndpoint`) и вызыватель по шине
(`.port`/`.emitter`). Третий — внешний: фронтенд, сервис на другом стеке,
скрипт. У него нет DI, и ему нужен HTTP-адрес операции.

Для внешнего потребителя контракт объявляет HTTP-адрес в секции `http:`, а
пакет `@nestling/client` строит из таких контрактов типизированный клиент
...
`makeContract`, `defineFail`, `Ok`/`Fail`, перечень статусов, формы io,
...
Импортируйте `makeContract` из `@nestling/contracts`. `@nestling/ports` его
не реэкспортирует: у этого пакета есть серверные зависимости.

## Адрес в контракте: секция `http:`
...
запись для контракта без пометок; разбирается она строго: ровно один
пробел, метод и путь непустые. Объект `{ method, path, bind?, rawBody?, sse? }`
нужен, когда есть `bind`, `rawBody` или `sse`.

Bind-карту `makeContract` вычисляет сразу, тем же кодом, которым
`httpEndpoint` обслуживает обычную HTTP-декларацию. Из этого следуют два
свойства.

- Все проверки размещения срабатывают при импорте модуля с контрактами, то
...
На вызовы по шине секция `http:` не влияет: вид контракта, `.port` и
`.emitter`, `implement` и вызов через порт работают как раньше. Один
контракт обслуживает и шину, и HTTP.
...
Контракт-форма — способ записи обычной HTTP-декларации, а не новый
примитив. `method`, `path`, `bind`, `rawBody`, `sse`, `input`, `output` и
`errors` берутся из контракта; повторное объявление любого из них здесь —
ошибка компиляции, как и в `implement`. Bind-карта берётся с контракта тем
же значением, а не вычисляется заново, поэтому сервер и клиент раскладывают
поля по одной и той же карте.

Контракт без секции `http:` отвергается в момент создания декларации.
Сообщение об ошибке называет контракт и предлагает два выхода: объявить
`http:` или реализовать контракт через `implement`, то есть по шине.
...
контракта (`'users.create'`) во вложенные объекты: имя контракта — адрес на
...
| Вид контракта | Вызыватель внутри приложения | Метод клиента |
|---|---|---|
...
Метод клиента возвращает то же, что `.port`: `Ok` или `Fail`, где множество
...
- Статус 2xx. HTTP-код переводится в `SuccessStatus`; `204` даёт
  `Ok('NO_CONTENT', null)`.
- Успешное тело проверяется схемой `output` контракта. Так по умолчанию;
...
- Статус не 2xx. Тело читается как `{ error, code?, details? }`. Если
  `code` объявлен в `errors:` контракта, клиент восстанавливает отказ по его
...
- Всё остальное даёт `UnknownError` с исходной ошибкой в `cause`: сеть
  ...
  в контракте, детали не прошли схему. Других кодов клиент не вводит.
...
вызове, описано в контракте: `input` и bind-карта.
...
- у контракта нет секции `http:`;
- контракт имеет вид `event`;
- у контракта потоковая (`stream`, `events`) или `multipart`-форма io;
  потоковый клиент проектируется отдельно;
- тело не JSON (`'binary'`, `'text'`);
- `baseUrl` не является абсолютным URL.

Всё, что можно проверить по записи контрактов, проверяется сразу; ошибок
...
- Не генерирует код: типы выводятся из контракта. Для потребителей не на
...
- [design/contracts.md](../design/contracts.md) — целевое состояние
  контрактов, портов и клиентов.
- [guides/ports.md](./ports.md) — вызов контракта изнутри приложения.
