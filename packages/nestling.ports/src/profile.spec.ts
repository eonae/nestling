 ...
 *
 * Канал провода (`raw.attributes`) проверяется тестами вызывателей и шины;
 * здесь — проекция для кода произвольной глубины: ambient-переменные, их
 * штатные писатели, policy-check присутствия и явная передача остатка
 * вложенному вызову.
 */
...
/** Субъект политики: та же форма, в какой его отдаёт дискавери */
const subject = (
...
describe('помощники бюджета', () => {
  ...
  it('без бюджета сигнал вызывающего уезжает как есть — без композиции', () => {
...
describe('Deadline — чтение бюджета из глубины', () => {
  ...
  it('без писателя в пайплайне переменной не существует вовсе', async () => {
    ...
    // Провод бюджет привёз, но проекции без писателя нет: диагностика
    // называет починку — композировать слой и объявить инвариант
...
describe('IdempotencyKey — чтение ключа из глубины', () => {
  ...
  it('команда без ключа вызывающего доезжает с ключом вызывателя', async () => {
