...
поэтому второго хранилища состояния не появляется. Единственный канонический
писатель — `Var.provide(compute)`, возвращающий обычный pre-юнит: накопительная
типизация и проверки композиции остаются прежней машинерией пайплайна, а
декларация переменной не может разойтись с фактом записи. Well-known
