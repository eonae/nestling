...
- Входная цепочка может менять тип, выходная — только `T → T`. Входная
  цепочка — видимая часть контракта: схема описывает данные в сети, а
...
- Наблюдение, лимиты, таймауты и фильтрация сохраняют тип и относятся к
  ...
  контракта: они допустимы только там, где видны в декларации.
...
контрактах. Ядро о нём не знает.
...
- Управление локально для узла, наблюдение — по кластеру. `list()` и
  `abort()` действуют в своём процессе. `event`-контракты
