...
Внутри слоя четыре шага. Первый берёт соединение из контейнера и шлёт
...
Первый шаг — класс, потому что кладёт в контекст не значение
...
`makeDrizzlePg` рядом с DI-токеном соединения и наружу не виден.
...
слоя транзакции, останавливает сборку на фазе BUILD — до открытия
