...
- [ ] 6.1 `packages/examples.app-with-http`: доменные отказы через
      `defineFail`, объявление в `errors:`, замена анонимных `Fail.*`
- [ ] 6.2 `packages/examples.simple-http-server` и
      `packages/examples.simple-cli`: то же
- [ ] 6.3 Показать в одном из примеров оба канала — `return` отказа и
      `meta.fail(...)` — и `.catch`-юнит, матчащий отказ по `.is()`
- [ ] 6.4 Прогнать примеры руками (запуск + запрос), убедиться, что коды
