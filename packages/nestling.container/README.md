...
Компилятор проверяет, что типы, порядок и длина списка зависимостей
совпадают с параметрами конструктора, а класс с токеном интерфейса
реализует этот интерфейс. Допустимую длину задаёт
`ConstructorParameters<T>`: у конструктора с необязательным параметром
годятся обе длины, у конструктора с rest-параметром — любая. Лишний
токен даёт диагностику `Dependency list is longer than the constructor
parameter list` с ожидаемой и фактической длиной.
