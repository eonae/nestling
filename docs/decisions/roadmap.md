...
| 76 | `errors-of` | — | хелпер `errorsOf(Operation)`: список отказов вызывающей операции — производное значение, а не копия чужого `errors:` | S | план — [ideas.md [2026-09-12]](./ideas.md) «Разбор обзоров d/10 и d/13»; d/13 §4.1, §6.6: на пяти операциях копирование — дисциплина, на пятидесяти — то, чего компилятор требует, но в чём не помогает |
