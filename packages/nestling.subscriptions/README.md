...
> Гайд: [глава 24. Кто сейчас подключён и как его отключить](../../docs/guide/24-ops.md).
...
Число прочитанных элементов на снятие не влияет: ответ, закрытый
транспортом до первого элемента, снимает запись так же, как дочитанный до
конца. Ядро выполняет `.finally` при любом закрытии потокового ответа.
