...
по сработавшим триггерам двух записей [deferred.md](./deferred.md).
...
| 77 | `observability` | — | W3C trace-context через `propagate`; семейство `Metrics$` в ядре с пустой реализацией по умолчанию, сателлит подменяет корень; счётчик и длительность на endpoint и вызов порта из `.finally` с `outcome` | M–L | план — [ideas.md [2026-09-12]](./ideas.md) «Разбор обзоров d/10 и d/13»; d/10 §3.3: фреймворк продаёт split-развёртывание через NATS, а трассировки и метрик нет вовсе |
