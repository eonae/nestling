...
Сахар `Family.auto` для случая «токен семейства именуется своим потребителем»
(канонический пример — логгер со скоупом класса). Резолюция сентинела в
конкретный токен семейства происходит в момент декорирования любым из трёх
...
уже резолвленный токен семейства. Резолюция SHALL происходить в момент
регистрации (декорирования), без какой бы то ни было рантайм-резолюции;
создание полученного токена семейства узлом графа на `build()` подчиняется общим
...
- **THEN** в конструктор инжектится токен семейства `"Logger:CreateUserEndpoint"`,
...
- **THEN** в `acquire` приходит токен семейства `"Logger:Database"`
