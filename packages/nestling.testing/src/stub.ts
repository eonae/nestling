 ...
 * контракта** на каждом вызове. Мок, разъехавшийся с реальностью, падает в
 * тесте, а не в проде.
 */
 ...
 *
 * Ровно то же множество, что закрывает `KernelPortFail`: страж границы
 * считает эти коды контрактными для кого угодно, поэтому и фейк вправе
 ...
 * разъезжался с прод-поведением ровно в том месте, ради которого ключ
 ...
 * @throws {TypeError} Если первым аргументом приехал не контракт
 *
 * @example
 * ```typescript
 ...
 * подменил контракт, и отчёт обязан это показывать. Обычные пары «токен →
 * значение» в состав не входят — это не контракты.
 */
