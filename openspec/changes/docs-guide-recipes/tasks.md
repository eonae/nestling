...
- [x] 6.1 Завести `scripts/site/sections.mjs`: список источников двух
      видов — папка с оглавлением и отдельная страница
- [x] 6.2 Заменить константу `GUIDE` в `build.mjs` обходом списка;
      проверка «состав README равен файлам папки» выполняется для каждой
      папки отдельно
- [x] 6.3 Группы сайдбара: у папки без заголовков `## Часть N.` — одна
      группа с заголовком из `sections.mjs`; страница даёт один пункт
- [x] 6.4 Пересчёт ссылки вне источников относительно `docs/.site/`
      вместо дословного переноса
- [x] 6.5 Ссылка на файл другой папки-источника переписывается в якорь
      по имени файла
- [x] 6.6 `yarn docs:dev` следит за всеми источниками из `sections.mjs`
- [x] 6.7 Обновить `scripts/site/README.md`

## 7. `docs:audit` и линтер стиля

- [x] 7.1 Проверка плашки «сверено с кодом» распространяется на
      `docs/recipes/*.md`; исключение `appendix-*` удаляется
- [x] 7.2 Проверка полноты оглавления работает для обеих папок
- [x] 7.3 Новая проверка: нумерация глав пути сквозная и без пропусков
- [x] 7.4 Ссылка плашки README пакета принимается в `docs/guide/` или в
      `docs/recipes/`
...
- [x] 8.1 Пометить запись [ideas.md [2026-09-05]](../../../docs/decisions/ideas.md)
      «Структура документации» как реализованную в части волны 3: что
      вышло целиком, что уточнилось, что осталось волнам 4 и 5
- [x] 8.2 Записать в ideas.md два уточнения решения: глава 12 не делится
      (повод исчез по замеру), глава о базе делится и встаёт на путь
- [x] 8.3 Обновить статус строки 59 `docs-guide-recipes` в
      [roadmap.md](../../../docs/decisions/roadmap.md)

## 9. Проверка

- [x] 9.1 `yarn docs:build` собирается: разделы на все главы, рецепты и
      страницы корня, ссылки в `design/` из обеих папок и из корня ведут
      в существующие файлы
- [x] 9.2 `yarn docs:audit` — 0 ERROR
- [x] 9.3 `node .claude/skills/docs-style/scripts/lint.mjs` — 0
      запрещённых слов
- [x] 9.4 Сверка вручную: ни одна ссылка `docs/guide/NN-` не ведёт на
...
- [x] 10.4 `design/` и `decisions/` синхронизированы по правилам
      `CLAUDE.md`
- [x] 10.5 Запись `ideas.md`, по которой шёл change, несёт пометку
      «РЕАЛИЗОВАНО»
- [x] 10.6 `yarn docs:audit` — 0 ERROR
- [x] 10.7 Затронутые `examples/*` мигрированы, главы пересверены с
      обновлённой датой в плашке «сверено с кодом»
- [x] 10.8 Коммиты осмысленные; push делает пользователь — из сессий он
      не проходит
