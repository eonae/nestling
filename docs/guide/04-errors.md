...
> `HttpHandler<Op>`, `HttpResponse`; `Ok` без заголовков; шаги
> транспорта».
...
возвращает отказ значением, как обычный результат. Клиент получает
документ отказа:
...
Тело отказа — документ RFC 9457 под медиатипом
`application/problem+json`. Машинный код отказа едет в члене `type` с
префиксом `urn:error:`, сообщение — в `detail`, детали — в `details`.
Такое тело разбирает готовая библиотека на любом стеке, а не код,
написанный под наш формат. `title` и `status` описывают сам ответ:
фразу статуса HTTP и его код. Форма одна на все отказы границы — и на
ответ хендлера, и на битый JSON, и на кадр ошибки в потоке.
