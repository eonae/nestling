 ...
 * сообщения или заголовком HTTP, арендатор конвертом. Слой возвращает их в
 * контекст запроса, и дальше их читает код любой глубины: `Ctx(TenantId)`
 * — прикладной, поле `traceId` записи — логгер ядра.
 *
 * Слой один на оба процесса. Именно поэтому записи процессов `users` и
 * `quotas` сходятся по одному `traceId`: обе стороны продолжают трассу
 * одним и тем же юнитом.
 */
