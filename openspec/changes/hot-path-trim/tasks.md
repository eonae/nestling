...
- [ ] 6.3 Замер после пункта с пайплайном из двух pre-юнитов и `.finally`
      (отдельный сервер `nestling-layers` в `servers.ts`, если нужен для
      замера) и без него; результат в таблицу

## 7. Документация и запись

- [ ] 7.1 `docs/decisions/ideas.md`: запись [2026-09-05] «Горячий путь:
      ...
      не трогали
- [ ] 7.2 `scripts/bench/README.md`: числа в разделе «Почему Nestling
      медленнее Fastify» и список оставшихся причин
- [x] 7.3 `docs/decisions/roadmap.md`: строка 38 `hot-path-trim`
- [ ] 7.4 Линтер `docs-style` по изменённым текстам — 0 запрещённых слов

## 8. Проверка

- [ ] 8.1 `yarn verify` зелёный под Node 24
- [ ] 8.2 `yarn docs:audit` — 0 ERROR
...
- [ ] 9.6 Затронутые `packages/examples.*` мигрированы, гайды пересверены
