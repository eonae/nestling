...
Существующие подсказки SHALL сохраняться: DI-токен, похожий на токен семейства,
...
- **WHEN** недостающий DI-токен выглядит как токен семейства, созданный
  `makeToken` вручную
- **THEN** сообщение по-прежнему объясняет, что токенов семейства создаёт вызов
  семейства
