...
построен логгер ядра из главы [9](../guide/09-logging.md).
...
DI-токен. Токен семейства работает везде, где работает обычный DI-токен: в
...
токеном семейства не является: контейнер сообщает об отсутствующем
...
интерфейса из главы [6](../guide/06-repository.md): семейство вызывается как
...
этого не нужно. Рядом стоит `Logger$('users')` — токен семейства логгера
...
каждому модулю; плагины описаны в главе [14](../guide/14-features.md).

При `build()` контейнер делает четыре шага.

1. Собирает токенов семейства, упомянутых в `deps` всех зарегистрированных
   провайдеров.
...
метод проверки. Имя проверки задаёт токен семейства, поэтому в самом классе
имени нет:
...
прогон. Обычному приложению этого достаточно — рецепт [«Кто сейчас
подключён и как его отключить»](./ops.md) показывает пробы на HTTP.
...
`HealthCheck$.all` стоит в `deps` рядом с узлом и токеном семейства
...
[14](../guide/14-features.md): `UsersModule` зависит от `DatabaseModule`, а
...
```bash
yarn start:dev
# граф с токенами семейств в браузере
yarn export-metadata
yarn visualize
```

Тот же пример читает конфиг из нескольких источников и меняет значения
без перезапуска — рецепт [«Конфиг из файла и без
перезапуска»](./config-sources.md).
