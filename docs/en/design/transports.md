...
`makeDispatch(endpoints, { logger })` gets it once from the build,
...
BUILD (`App` knows both the declarations and the transport
...
steps ([pipeline.md](./pipeline.md)). On detecting a disconnect, the
...
`.finally` steps run on close regardless of how many items were read
...
on BUILD reads the declaration, because there is no instance yet on
this phase.
...
build matches them against the capabilities of the transport. A
declaration with a shape the transport does not support fails before
it accepts requests: on phase BUILD under `App`, and in `serve` on
...
### 1.2 Start context and transport steps
...
`rawBody` and `lastEventId` to it. A step typed by this context knows
...
`implement` does not, and the compiler will not let an HTTP step into
the implementation of an operation on the bus. A transport supplies
such steps through exports: `withHeader(name)` puts a header into the
context, `withClientIp()` puts the address of the client,
`httpAccessLog(logger)` writes an access line with `bytesIn` and
`bytesOut`. The logger arrives as an argument, so the step stays a
function and does not grow the pipeline's `TNeeds`. A transport never
attaches steps implicitly: the layer is always visible in the
...
where the `withTracing()` step reads it ([pipeline.md §3](./pipeline.md)).
...
built only from canonical places, with the priority "path, then
...
transport references it through the `server` field, and the build
...
`stream` or `events` is rejected on BUILD by the same shape check.
...
A declared bus with no assigned role is a build error: the
...
stream and both numbers — one per stream. There is no build
...
converter. Uniqueness of the name is checked by the kernel on BUILD as
