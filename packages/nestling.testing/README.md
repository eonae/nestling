...
| экземпляры и захват ресурсов в топологическом порядке | обработчики `SIGTERM`/`SIGINT` |
...
(соединение с БД, клиент брокера), объявляйте ресурсом.

**Фаза тестового прогона — `'RUN'`.** Сборка останавливается после WIRE,
но `testApp.call` и есть приём запроса, поэтому узлам ядра тестовый корень
сообщает `'RUN'`. Без этого `readiness()` узла проб отвечал бы `not_ready`
в любом тесте, и проверить «проба зелёная, когда база жива» можно было бы
только сквозным тестом с сокетом.
...
- Тип `input` берётся из формы `input` декларации, тип результата — из
  формы `output`. Ветка отказа несёт `code` из эффективного множества
  отказов декларации — `errors:` плюс отказы слоёв её пайплайна и коды
  ядра, — а `status` равен категории этого кода.
...
```typescript
overrides: [
  [UsersRepository, inMemoryUsersRepo()],      // DI-токен и заглушка
...
- Пара типизирована: заглушка, не совпадающая с типом DI-токена, не
  компилируется.
- Подмена DI-токена, которого нет в графе, останавливает сборку.
  Переименуйте провайдер, и тест упадёт вместо того, чтобы молча ничего не
  подменять.
- Повторная подмена одного DI-токена останавливает сборку.
- Строковой формы (`overrideByName('…')`) нет: подменить можно только
  DI-токен, на который есть ссылка.
...
«DI-токен вызывателя и заглушка»: `[C.caller, …]` для `request`-операции,
...
одного плагина — с её модулями и их `dependsOn`, модулем ядра для конфига и
перечисленными заглушками; те же фазы 0–3, тот же `TestApp`. Каждую
неудовлетворённую зависимость нужно заглушить явно. Ошибка перечисляет все
недостающие DI-токены и потребителя каждого, а не первый попавшийся. `stubs`
здесь означает «дать недостающее», а не «заменить существующее»; в это же
поле кладётся `stub(Operation, impl)`. Опции: `stubs`, `config`,
`transports`.
...
| `stub(operation, impl)` | пара «DI-токен вызывателя, заглушка» для `stubs:` |
