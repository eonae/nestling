...
- [ ] 8.4 Пользователь включает Pages с источником «GitHub Actions» в настройках репозитория — единственное действие вне репозитория (**за пользователем**: из сессии настройки репозитория не меняются)
- [ ] 8.5 Ручной запуск workflow проверяет выкладку до первого тега: сайт открывается по адресу, переключатель языка работает, поиск находит разделы своего языка (**за пользователем**: идёт после 8.4; локально вывод проверен — 145 страниц, 12 220 ссылок, ни одной битой, оба индекса поиска разрешаются от корня своего языка)
...
- [x] 9.6 ~~`docs/decisions/archlog.md`: абзац о публикации и двуязычии~~ — отменено: на `main` журнал заморожен и переехал в `docs/history/archlog.md`, его место заняла пометка «РЕАЛИЗОВАНО» в `ideas.md` по правилу 9

## 10. Definition of Done

- [x] 10.1 Все задачи выше отмечены, кроме 8.4 и 8.5 — они за пользователем
- [x] 10.2 `yarn verify` зелёный
- [x] 10.3 README затронутых пакетов обновлены, включая плашки статуса — все 21 стали парой, плашки ведут в `docs/en/`
- [x] 10.4 `design/` и `decisions/` синхронизированы по правилам `CLAUDE.md`; запись `ideas.md`, по которой шёл change, несёт пометку «РЕАЛИЗОВАНО» с шестью уточнениями и закрытыми вопросами 1 и 2
- [x] 10.5 `yarn docs:audit` — 0 ERROR (10 WARN `guide-stale` достались от `main`: четыре главы и рецепт ждут пересверки с примерами, которые менялись позже)
- [x] 10.6 `examples/*` change не затронул: код не менялся, мигрировать нечего. Даты плашек не двигались — перевод несёт дату оригинала
- [x] 10.7 `main` не тронут — слияние делает Merger после `/opsx:archive`
