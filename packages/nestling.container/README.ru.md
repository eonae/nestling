...
Обратная к рецепту задача: много независимо регистрируемых вкладов и один агрегатор, который состава не знает. **Вклад — обычный провайдер** с членским токеном, регистрируемый там, где ему место:
...
// api.module.ts — другой модуль, первый править не пришлось
providers: [classProvider(IHealthCheck('api'), ApiHealthCheck)],
exports: [IHealthCheck],
```

Агрегатор зависит от сентинела `IHealthCheck.all`, типизированного как `TokenString<readonly HealthCheck[]>`:
...
На `build()` — после фикспоинта материализации членов — контейнер регистрирует **синтетический узел-агрегат**: его deps это токены всех зарегистрированных членов семейства, а инстанс — массив их инстансов. Дальше это обычный узел: проверка циклов, топологические `init()`/`destroy()` (вклады инициализируются до потребителей агрегата и уничтожаются после них), `toJSON()`, визуализация, `strictExports`.

Правила, которые стоит знать:

- **состав** — все члены, у которых есть провайдер на момент создания агрегата: явные вклады, члены, материализованные рецептом, и члены из `.auto`. `familyProvider` не обязателен: семейство с одними явными вкладами агрегируется штатно;
- **`.all` не форсит материализацию** — член, созданный вызовом `IHealthCheck('orphan')`, но никем не запрошенный и без явного провайдера, в массив не попадает (и узла не имеет);
- **неупомянутый `.all` не создаёт узла** — `container.get(IHealthCheck.all)` вернёт `null`;
- **пустое семейство даёт пустой массив**, а не ошибку: «фичу не выбрали, её вкладов нет» — штатное состояние;
- **порядок — порядок регистрации**: модули и провайдеры в порядке регистрации, затем члены, добавленные фикспоинтом материализации. Он детерминирован, и больше ничего не обещано: если порядок несёт смысл (цепочка middleware), не опирайтесь на порядок `imports`;
- **массив `readonly` и заморожен** — это снапшот сборки, разделяемый всеми потребителями, и мутация одним была бы видна остальным;
- **агрегат не принадлежит ни одному модулю**, поэтому при `strictExports` его ребро на вклад модуля M требует, чтобы M его экспортировал; семейство в `exports` (`exports: [IHealthCheck]`) экспортирует всех членов разом;
- **токен зарезервирован**: провайдер с `provide: IHealthCheck.all` — ошибка, как и членский параметр `'{all}'`.
...
- `makeTokenFamily<T, [param: string]>(name): TokenFamily<T>` - Создать семейство токенов; `Family(param)` возвращает мемоизированного члена `"<name>:<param>"`, `Family.auto` — consumer-aware сентинел, `Family.all` (`TokenString<readonly T[]>`) — агрегирующий сентинел multi-injection
