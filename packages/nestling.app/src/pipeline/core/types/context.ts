...
export interface EndpointMeta {
  ...
  /** Конфигурация output: схема, примитив, stream-модификатор или развилка */
  output?: AnyOutput;

  /**
   * Объявленный статус единственного успешного исхода (`status:`
   * декларации).
   *
   * Его получает значение, возвращённое без обёртки `Ok`. Поля нет —
   * действует умолчание: `ok` при объявленном `output`, `no_content` без
   * него. Развилка объявляет статусы ключами, и поле рядом с ней не
   * заполняется.
   */
