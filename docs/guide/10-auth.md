...
> `.pre(step, { errors })`, канал `return` у pre-шага, эффективное
> множество `errors`» и «[2026-09-12] Транзакционный приём».
...
Отказ шага объявляется в точке подключения — вторым аргументом `.pre`.
Вернуть отказ вне этого списка нельзя: компилятор отвергнет шаг прямо в
...
pre-шага проходит ту же проверку, что и отказ хендлера:
незадекларированный отказ граница пайплайна заменяет на `internal_error`
с кодом `500`.

У pre-шага три исхода. Первые два видны здесь: добавка к контексту
(`{ caller }`) и отказ (`Unauthorized()`). Третий — досрочный успех: шаг
возвращает `done()`, и endpoint завершается успехом, не доходя до
хендлера.

Досрочный успех объявляется тем же вторым аргументом:
`.pre(step, { done: true })`. Декларация с таким слоем обязана быть без
`output` — значения досрочный успех не несёт, — и это проверяется при её
создании. Шаг, вернувший `done()` без объявления, роняет запрос ошибкой
с починкой в тексте.

Первым применением канала стал слой дедупликации на приёме сообщений
([глава 16](./16-durable-events.md)): повтор доставки
завершается успехом, а хендлер не вызывается.
...
Поля, которые pre-шаги положили в контекст, хендлер получает вторым
...
`Ctx(RequestId)`. Читатель отдаёт значение, которое положил pre-шаг слоя.
...
объявлял эту переменную. Объявлением считается pre-шаг вида
`Var.provide(…)`; `withRequestId()` из главы 9 — такой шаг. Шаг, который
...
называет плагин ([глава 16](./16-durable-events.md)).
...
Шагу, которому нужен запрос, транспорт даёт стартовый контекст
`HttpStartContext`:
...
шаги не приставляет — слой всегда виден в декларации.
