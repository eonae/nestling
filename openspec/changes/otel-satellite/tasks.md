...
- [ ] 4.6 Открытый вопрос 2 дизайна: имя `service` против `serviceName`;
  решение записать в `design.md`
- [ ] 4.7 Открытый вопрос 3 дизайна: сверить выбор `snapshot()` против
  `tap(sink)` с сессией `change/rewrite-metrics` и записать исход

## 5. Примеры

Экспозиция там уже подключена пакетом формата — этот change её не трогает.

- [ ] 5.1 `examples/microservice/src/observability.ts`: слой композирован с
  `telemetry.spans`; `src/app.ts` получает `otel(…)` и `hasVar(Span)` в
  `policies:`
- [ ] 5.2 `examples/modular-app/src/base.ts`: то же, слой в обоих процессах
- [ ] 5.3 Спека примера: участок, полученный подставным `SpanExporter`, и
  общий `traceId` у двух процессов
- [ ] 5.4 Своего `.finally`-шага с отправкой участка в `examples/` не
  осталось (grep пустой)
- [ ] 5.5 README обоих примеров: строка про экспорт трассы
- [ ] 5.6 `yarn verify` зелёный по обоим примерам
...
- [ ] 6.2 Оглавления `docs/guide/README.md` и `docs/en/guide/README.md`:
  строка главы 23
- [ ] 6.3 `docs/guide/22-metrics.md` и пара: абзац про push по OTLP рядом с
...
- [ ] 6.4 `docs/design/container.md` и пара: строка про экспорт участков
...
- [ ] 6.6 `docs/README.md`: строка `@nestlingjs/otel` в таблице пакетов
- [ ] 6.7 `docs/compatibility.md` и пара: строка про экспорт телеметрии
- [ ] 6.8 `docs/glossary.md` и пара: термин «участок трассы» уже есть —
...
- [ ] 7.2 Запись `ideas.md` [2026-09-12] «Разбор обзоров d/10 и d/13»
  получает пометку «РЕАЛИЗОВАНО» по пункту 2 в части сателлита
- [ ] 7.3 Оглавление `ideas.md` обновлено
  (`node .claude/skills/docs-audit/scripts/ideas-toc.mjs`)
- [ ] 7.4 Если участок на вызов порта решено делать позже — строка в
