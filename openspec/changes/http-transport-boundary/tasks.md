...
- [ ] 1.1 Свести список текущих экспортов `index.ts` (`parser.ts`,
...
- [ ] 3.1 Переписать раздел «Границы пакета» README: обещания (HTTP/1.1,
      ...
      аргументов
- [ ] 3.2 Плашка статуса README сверена с фактическим состоянием пакета
- [ ] 3.3 Линтер `docs-style` по README пакета — 0 запрещённых слов

## 4. Бенчмарк относительно Fastify

- [ ] 4.1 `fastify` и `autocannon` — devDependencies корневого
      `package.json`, не пакета `transport.http`
- [ ] 4.2 `scripts/bench/http-vs-fastify.ts`: одинаковая пара endpoint'ов
      на обоих серверах — `GET /users/:id` (JSON-ответ без валидации
      входа), `POST /users` (тело, проверенное zod-схемой на обеих
      сторонах)
- [ ] 4.3 Скрипт запуска в `package.json` корня (например `bench:http`),
      вне `yarn verify`
- [ ] 4.4 Прогнать бенчмарк локально, зафиксировать условия замера (Node,
      ОС, дата)
- [ ] 4.5 Дополнить запись `docs/decisions/ideas.md` [2026-09-04] новым
      абзацем с результатом и условиями замера (append к записи, без
      правки существующего текста)

## 5. Проверка

- [ ] 5.1 `yarn verify` зелёный по `@nestling/transport.http` и
      зависящим пакетам
- [ ] 5.2 Линтер `docs-style` по изменённым текстам (README пакета) — 0
      запрещённых слов
- [ ] 5.3 `yarn docs:audit` — 0 ERROR
