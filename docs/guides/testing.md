...
Фича, которая инжектит `ClaimQuota.port`, без соседа не соберётся:
контейнер проверяет, что у контракта есть реализация в этой сборке или
...
`stub(C, impl)` возвращает пару «токен вызывателя → фейк»: `[C.port, …]`
для `request`-контракта и `[C.emitter, …]` для `command` и `event`.
Сторону выбирает вид контракта. Пара передаётся в `stubs:` наравне с
обычными парами «токен → значение»; отдельного поля для контрактных стабов
нет.

Механизм простой: явный провайдер члена семейства имеет приоритет над
рецептом семейства. Поэтому для застабанного контракта боевой вызыватель
не строится, и проверка достижимости не выполняется.

**Фейк проверяется схемами своего контракта при каждом вызове.**
...
- Отказ обязан входить в `errors:` контракта или быть одним из kernel-кодов
  (`VALIDATION_FAILED`, `UNKNOWN`, `DEADLINE_EXCEEDED`). Отказ с другим
  кодом — дефект теста: стаб бросает ошибку с именем контракта, полученным
...
`app.emit(Contract, payload, meta?)`: он доставляет событие или команду
...
- `request`-контракт передать нельзя: это ошибка компиляции. У `request`
...
контракта. Это их назначение, но из-за них app-тест не доказывает, что
...
`app.stubbed` — имена застабанных контрактов по алфавиту, симметрично
`app.pruned`. `report.contracts` каждой топологии — дескрипторы контрактов,
которые она публикует. Сравнение этих двух списков — машинная форма
правила: стаб, который прикрывает контракт без реализации ни в одной
...
### Отчёт совместимости контрактов

Отчёт каждой топологии содержит поле `contracts` — дескрипторы
контрактов, которые эта топология публикует. Поэтому проверка «не сломал
ли я контракт соседней фичи» пишется одним импортом и без пересборки
приложения:
...
строятся: структурная часть (вид контракта, формы io, коды и статусы
отказов) точна, а листовые схемы помечаются непрозрачными и получают
вердикт `unknown`.

Матрица сводится объединением: контракт, который публикует только
топология `all`, в снапшоте есть и знает, кто его опубликовал. Иначе фича,
не выбранная в топологии `ops`, выглядела бы как удалённый контракт.

Падает здесь только `expect` теста: `diffContracts` — чистая функция двух
...
что автор модуля держит в контракте с реализацией: в примере это токен
...
## 8. Модуль в изоляции: `testModule`
...
`testModule` поднимает мини-приложение из одного модуля с его `imports` и
kernel-модуля конфига. Поле `stubs` то же, что у `assembleTest`:
контрактный вызов, объявленный модулем, поставляется им наравне с
...
позиции работает `jest.fn()`. `app.port(Contract)` — типизированный порт
