...
- **Литерал `http` на каждый HTTP-запрос удорожает горячий путь.** →
  ...
  Замер уложился в порог 2% (см. «Замер» ниже), но не сам собой: цену
  дали две вещи, которых в первой версии не было видно.

  - `socket.remoteAddress` идёт в libuv. Читать его на каждый запрос
    стоило 3% на `GET`, поэтому `ip` — аксессор, а запрос в контексте
    стал классом: аксессор в литерале объекта отнимал уже 10%, потому что
    V8 создаёт такой объект медленным путём.
  - `Date.now()` на каждый запрос стоил 2.4% на `GET`. Часов на запросе
    не осталось, и `httpAccessLog` пишет строку доступа без
    длительности — так же, как её описывает
...
`yarn bench:http` под Node 24.10.0 (macOS 25.3.0 arm64, Apple M1 Max × 10,
50 соединений, 8 c на замер). Сравнивались сборки `main` и ветки change'а,
чередуясь по 6 раундов; в таблице среднее req/s.

| Сценарий | до | после | Δ |
| --- | ---: | ---: | ---: |
| `GET /users/:id` | 56303 | 55875 | −0.8% |
| `POST /users` | 47109 | 46567 | −1.2% |

Порог — не хуже 2% — выдержан на обоих сценариях.
