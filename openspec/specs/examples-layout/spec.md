...
иллюстрации к главам гайда, они существуют только внутри репозитория и в
npm не публикуются. Поэтому они лежат в отдельном каталоге `examples/`, а
`packages/` содержит только пакеты фреймворка. Запрет на публикацию держит
