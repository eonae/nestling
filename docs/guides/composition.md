...
Вызов виден и в `deps` декларации, и в зависимостях обычного провайдера
фичи. Не виден он только в модуле, чьи `providers` заданы фабрикой: её
вызывает `build()`, то есть уже после выбора.
