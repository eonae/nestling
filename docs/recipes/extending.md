...
> Гайд по текущему API; сверено с кодом `a2535b50`.
...
через `buildTest` и не тянет в приложение ни вендора схем, ни
хранилища.

Такой пакет называется сателлитом: он собран поверх публичных примитивов
ядра и живёт вне него. Образец в этом рецепте — `@nestlingjs/subscriptions`,
реестр подписок из рецепта [«Кто сейчас подключён и как его
отключить»](./ops.md).
...
| `@Handler`, `resourceProvider` | `@nestlingjs/container` | реестр как ресурс графа, класс-шаги слоя |
...
Тот же замер прошли ещё два сателлита, потяжелее: `@nestlingjs/outbox`
и `@nestlingjs/drizzle.pg` из глав [11](../guide/11-database.md) и
[16](../guide/16-durable-events.md).
Правок ядра не понадобилось и им, но места, в которые они упёрлись,
остались: каждый пакет держит их живыми тестами в `core-limits.spec.ts`,
а перечень с ценой лежит в
[decisions/ideas.md](../decisions/ideas.md).
...
Слой состоит из двух шагов в форме класса: обоим нужен реестр из
контейнера. Pre-шаг возвращает поле `subscription`, и хендлер видит его
в типах как `meta.subscription`. Шаг `.finally` для потокового `output`
...
[главе 17](../guide/17-live-feed.md): `push` не ждёт наблюдателей, медленный
...
[главы 20](../guide/20-split.md), положите их в subpath `./testing` под условием
...
**объявление**: поле `capabilities` у `TransportDeclaration`. Так
проверка форм идёт на фазе BUILD, где экземпляров ещё нет. Декларация,
форма io которой не входит в способности транспорта, отклоняется до
обслуживания первого запроса. На транспорт ссылаются DI-токеном
экземпляра, а объявление для словаря `transports:` даёт
`transportValue(token, instance, { capabilities })`. Реальные транспорты
`http()`, `cli()` и `nats()` построены на том же интерфейсе и кладут в
объявление свою константу способностей.
...
Это последний рецепт списка. Целевое описание каждой подсистемы лежит в
...
[приложении А](./alternatives.md); соответствия понятиям
NestJS в [приложении Б](../from-nestjs.md).
