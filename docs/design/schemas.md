...
знания структуры схемы, устроено явно: bind-карта пишется руками при
непрозрачной схеме ([endpoints.md](./endpoints.md)), генерация JSON Schema —
через вендор-конвертеры (ниже).
