...
/**
 * Предел числа раундов создания токенов семейств в `build()`.
 *
 * Рецепт, чей провайдер зависит от нового DI-токена того же семейства, порождал
 * бы токенов семейства бесконечно; предел превращает это в ошибку сборки.
 */
...
interface FamilyRecipeEntry {
  /** Возвращает определение провайдера для одного токена семейства */
...
export interface ContainerBuilderOptions {
   ...
   * Поле тестовой сборки: `build` его не передаёт.
   * См. {@link TokenOverride}.
   */
  overrides?: readonly TokenOverride<any>[];

  /**
   * Рецепты семейств, подменяемые до создания DI-токенов.
   ...
   * Сборка приложения передаёт сюда карту фазы BUILD.
   */
...
export class ContainerBuilder {
  ...
  /**
   * Перечисляет зарегистрированных токенов семейства DI-токенов.
   *
   ...
   * токена семейства: `Family.all` отдаёт значения, а имя токена семейства живёт в его DI-токене.
   *
   * Токены семейства, которых создаст рецепт семейства в `build()`, сюда не входят:
   * они появляются от спроса, а спрос известен только внутри `build()`.
   *
   * @template T - Тип значения токена семейства
   * @param family - Семейство, созданное `makeTokenFamily`
   * @returns токены семейства в порядке регистрации
   */
  familyMembers<T>(
   ...
   * 2. подменяет рецепты семейств из `familyOverrides` — до создания DI-токенов;
   * 3. создаёт токенов семейств, упомянутых в зависимостях;
  ...
  build(): BuiltContainer {
    ...
    // Шаг 2: подменить рецепты семейств — строго до создания DI-токенов, иначе
    // токены семейства создались бы по боевому рецепту
    this.applyFamilyOverrides();

    // Шаг 3: превратить упомянутых токенов семейств в обычные провайдеры
    ...
    // прунинга, чтобы агрегат собрался из оставшихся токенов семейства
  ...
  private resolveProvider(plainOrCls: Provider): ProviderDefinition {
    ...
    // Позиция `providers:` принимает любую объявленную роль: класс-шаг
   ...
   * отдельно и используется при создании токенов семейства в `build()`.
   */
  ...
  /**
   * Превращает каждого упомянутого токена семейства в обычный провайдер.
   *
   * Выполняется после разворачивания фабрик и до создания экземпляров.
   * С этого момента токены семейств неотличимы от провайдеров,
   * зарегистрированных вручную: циклы, хуки жизненного цикла и привязка к
   * модулю работают для них так же.
   *
   * Провайдер, который вернул рецепт, сам может зависеть от токенов семейства
  ...
  /**
   * Собирает токенов семейств, упомянутых в зависимостях провайдеров, у
  ...
  /** Вызывает рецепт семейства для одного DI-токена и регистрирует результат. */
   ...
   *
   * Выполняется строго после создания токенов семейства (состав известен, только когда
   ...
   * `toJSON()`, визуализация.
   *
   * Повторять сбор токенов семейства после этого шага не нужно: зависимости агрегата —
  ...
  private materializeFamilyAggregates(): void {
    ...
    // Только по зависимостям, как и токены семейств: агрегат, которого никто
   ...
   * зарегистрированных токенов семейства.
   *
  ...
  /**
   * Возвращает DI-токены зарегистрированных токенов семейства в порядке
   * регистрации.
   *
   * `#providers` хранит порядок вставки, поэтому сначала идут явные
   * провайдеры в порядке регистрации модулей, затем токены семейства, созданные
   * рецептом, в порядке раундов. Токенство семейства читается полем DI-токена, а не
   ...
   *
   * Выполняется до создания DI-токенов: токен семейства, созданный боевым рецептом, уже
  ...
  private applyFamilyOverrides(): void {
    ...
    for (const { family, recipe } of this.#familyOverrides) {
      ...
      // Модуль боевого рецепта сохраняется: токены семейства остаются привязаны к
  ...
  /**
   * Разворачивает ребро на `Family.all` в рёбра ко всем токенам семейства.
   *
   * Во время прунинга узла-агрегата ещё нет (он создаётся из оставшихся
   * токенов семейства после), поэтому без разворачивания потребитель `Family.all`
   * потерял бы ровно тех токенов семейства, которых запросил.
   */
  ...
  private appendFactoryProviders(): void {
    ...
    for (const [moduleName, factory] of this.#providersFactories.entries()) {
      ...
      if (isThenable(providers)) {
        throw new Error(
          `Providers factory of module '${moduleName}' returned a Promise, but build is synchronous and does no I/O. Build the list of providers without reading files, network or connections.`,
        );
      }
 ...
 *
 * Отдельный случай — токен семейства: он становится узлом графа, только
 * когда кто-то его инжектирует, поэтому «не зарегистрирован» выглядит как
 * опечатка, хотя на самом деле «этого токена семейства никто не запросил».
 */
 ...
 * там нет имени, по которому можно выбрать токена семейства.
 */
