/**
 * Провод: кодек тела и конверт в headers.
 *
 ...
 * (`timeoutMs`, `idempotencyKey`, провозимый контекст), и он едет
 ...
 * `structuredClone`, которым репетирует провод режим `always-remote`
 * (`Date` уезжает строкой, `Map`, `Set` и `undefined` теряются). Молчаливой
 * потери при этом не бывает: вход валидируется схемой контракта **на
 * приёме**, поэтому поле, сжавшееся до строки, даёт отказ валидации с
 * именем поля.
 */
 ...
 * заголовка провод не переживает, а ключ ambient-переменной обязан
 * доехать буквально — он же имя поля накопленного `input`. Значение
 ...
 * доехать буквально (см. {@link CONTEXT_HEADER}).
 *
...
export function decodeEnvelope(headers?: NatsHeadersLike): WireEnvelope {
  ...
  if (headers.has(CONTEXT_HEADER)) {
    try {
      ...
      /* нечитаемый контекст просто не доезжает: терять сообщение незачем */
    }
  }
