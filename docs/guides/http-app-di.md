...
> В примере ручки `GetUser` и `CreateUser` объявлены **контракт-формой**
> (`httpEndpoint({ contract, … })`): адрес, схемы и `errors:` живут в
> контракте `src/api.contracts.ts`, который импортирует и внешний клиент.
> Анонимная форма выше — то же самое без второго потребителя; обе формы
> дают обычную HTTP-декларацию. См. [`typed-client.md`](./typed-client.md).
