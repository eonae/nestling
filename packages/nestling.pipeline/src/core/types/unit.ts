...
/**
 * Тип input для юнитов ответной фазы: поля, добавленные собственными
 * pre-юнитами слоя, опциональны (обогащение могло не случиться — pre
 ...
 * накопленный контекст (успех гарантирует, что все pre-юниты прошли).
...
/**
 * Catch-юнит: вызывается только для ответа-ошибки. Поля, добавленные
 * pre-юнитами собственного слоя, — Partial. Может заменить ошибку
