...
**Перечень полей закрыт.** Отдельного bag'а под сквозную инфраструктуру
(`plugins:` или что бы то ни было с той же ролью) не существует:
процесс-глобальная инфраструктура приезжает теми же `modules:`/`providers:`,
что и всякая другая, а feature-scoped — модулями своей фичи (§5). Второе поле
с той же семантикой заставляло бы выбирать между ним и `modules:` на каждом
инфраструктурном модуле; `providers:` живёт рядом по другой причине — он
выражает другую **форму** (провайдер, а не модуль).
...
## 5. Инфраструктура и cross-cutting

Примитива «плагин» в ядре **нет**: ни типа, ни поля корня, ни хука
конфигурации, ни реестра поднятой инфраструктуры. Логирование, трассировка,
клиент внешнего сервиса, шина — обычные модули и провайдеры. Разложение
привычного плагина по механизмам фреймворка:

| Часть плагина | Механизм nestling |
|---|---|
| «модули плагина» в корне | `modules:`/`providers:` корня — процесс-глобальная инфраструктура; `modules:` фичи — feature-scoped |
| параметризация плагина | функция, возвращающая модуль: `logging({ … })`; значение создаётся один раз и импортируется ([container.md](./container.md)) |
| конфиг плагина | секция `makeConfig`, объявленная самим инфра-модулем; наружу — только `.keys` ([config.md](./config.md)) |
| «только для этих транспортов» | обычная зависимость от токена транспорта → fail-fast на ASSEMBLE |
| «плагин, знающий состав приложения» | зависимость от `Discovery$` — состав как обычный узел графа (ниже) |
| ambient middleware | экспортируемый pipeline-слой + `everyEndpoint(…).hasLayer(ref)` ([pipeline.md §7](./pipeline.md)) |
| форматирование и отправка ошибок | `.catch`/`.finally`-юниты того же слоя ([errors.md](./errors.md)) |
| обогащение контекста | `pre`-юнит слоя; чтение из глубины — ридеры асинхронного контекста |

**Состав приложения — значение в графе.** `assemble` регистрирует результат
дискавери провайдером-значением под токеном `Discovery$` — всегда и без
условий: провайдер-значение ничего не стоит, а условная регистрация сделала
бы satellite-модуль зависимым от флага в корне. Значение то же самое,
которое `App` вычисляет до построения графа: второй дискавери не выполняется.
Это единственный способ модулю увидеть **выбранную** топологию, не дублируя
`select` в корне — а дублирование давало бы два разъезжающихся списка, и
заметить расхождение было бы нечем. Значение **read-only** (замороженные
списки, мутаторы карты подменены броском): инжектируемая дискавери это
поверхность интроспекции, а не точка расширения — состав приложения
определяется деревом модулей, а не графом. Первый потребитель — генерация
документации ([schemas.md §2.1](./schemas.md)).

**Параметр модуля против конфиг-секции.** Параметр функции-модуля —
решение **composition root**: какие провайдеры существуют, какая реализация
выбрана, как назван инстанс. Оно известно в момент сборки и видно в диффе
ревью. Конфиг-секция, которую модуль везёт с собой, — решение **среды**:
адреса, порты, уровни, таймауты; она приезжает из источников, валидируется на
`build()` и может быть reloadable. Критерий для автора: *меняется без
пересборки образа — секция; меняет то, какие провайдеры существуют, —
параметр.*

**Feature-scoped против ambient.** Инфраструктура, импортированная фичей,
приезжает вместе с выбором этой фичи и отсутствует целиком, если фича не
выбрана, — даже когда её файлы импортированы процессом. Co-located фичи,
импортирующие **одно значение**, делят один инстанс; разнесённые по процессам
получают по инстансу на процесс. Ничего специального для этого делать не
нужно: так работают жадный контейнер и `select`. Процесс-глобальная
инфраструктура объявляется в `modules:`/`providers:` корня — и «глобальность»
означает только это: провайдер есть в графе, но инжектнуть его может лишь
тот, кто импортировал токен. Ambient-инжекта «по факту регистрации в корне»
не существует.

**Сквозное поведение — слой плюс политика.** Инфра-модуль экспортирует
pipeline-слой значением, ручки композируют его явно, а вездесущность
гарантирует инвариант на собранном графе:
...
Уровней App/module у пайплайна нет и не будет ([pipeline.md](./pipeline.md)):
«видимо скомпозировал и проверил» вместо «невидимо навесил на всех». Забытый
слой ловится на ASSEMBLE с именем ручки, а осознанное исключение пишется
причиной (`detached: '<почему>'`), которую видно в диффе и в отчёте
`check()`.
