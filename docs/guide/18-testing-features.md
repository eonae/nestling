...
или `vi.fn()`. Список застабанных операций доступен как
...
объединением опубликованных операций. Стаб снимает проверку достижимости
только в своём тесте: в матрице подстановок нет, и операция, которой в
топологии никто не отвечает, роняет её сборку — это и есть смысл правила
«мокаешь — проверь топологию».

## Подмена в app-тесте: контекст, граф, топологии

Хранилище читает `requestId` ридером `Ctx(RequestId)` из главы
[9](./09-logging.md). Ридер является узлом графа, поэтому подменяется тем
