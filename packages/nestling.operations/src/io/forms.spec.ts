 ...
 *
 * Не строгая идентичность: маппированный тип (`FilesOf`) и литерал,
 * написанный вручную, для пользователя взаимозаменяемы, но компилятор не
 * считает их идентичными. Проверяется то, что видит автор хендлера.
 */
type Equals<A, B> = [A] extends [B] ? ([B] extends [A] ? true : false) : false;
function expectType<T extends true>(assertion?: T): void {
  // Проверку делает компилятор; параметр нужен, чтобы тип-аргумент был
  // использован
  void assertion;
}

/**
 * Функция-фабрика цепочки: форму можно собрать функцией и переиспользовать,
 * потому что комбинаторы возвращают новое значение.
 */
...
describe('вывод типов payload по форме', () => {
  it('схема без обёртки выводится как значение', () => {
...
describe('проверки конструкторов форм', () => {
