...
| 18 | `testing-package` | `@nestling/testing`: `assembleTest` (`overrides` только в тестовом корне; подстановка на ASSEMBLE + прунинг осиротевших поддеревьев; фазы 0–3 без START → in-proc `app.call`/`app.emit` по схемам; `await using` → SHUTDOWN), `vars()` (объектный `ConfigSource` c `watch`/`set`), `stub(Contract, impl)` (фейк-порт, валидируемый схемой контракта), `familyOverride`, `.check()` (фазы 0–1, матрица `select`-топологий в CI), `testModule()`; конвенция `./testing`-subpath (conditional export) | M | **ядро done** — [архив](../../openspec/changes/archive/2026-07-31-testing-package/), новый пакет [`@nestling/testing`](../../packages/nestling.testing/), [ideas.md [2026-07-10]](./ideas.md); `stub(Contract)` **и `app.emit`** — остаток в волне 6, после 11 |
...
| 26 | `contract-versioning` | версия явно в имени контракта; `describeContract` → дескриптор-значение (листья — через `SchemaDocConverter`, без конвертера честно непрозрачны); `snapshotContracts` сводит `.check()`-матрицу объединением топологий; `diffContracts` с закрытым словарём `breaking`/`additive`/`unknown` и направлением по слоту; отчёт-значение плюс `formatCompatibility` и подсказка bump'а — подсвечивает, не блокирует (флага блокировки не существует) | S–M | **реализован**, ждёт `/opsx:archive` — [гайд](../guides/ports.md), [ideas.md [2026-07-31]](./ideas.md), [ideas.md [2026-07-13]](./ideas.md) |
...
| 26 | `contract-versioning` | S–M | **реализован**, ждёт `/opsx:archive`; снапшот из дискавери, дифф с тремя вердиктами, отчёт в `.check()`-матрице (18) |
