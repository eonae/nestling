 ...
 *
 * Источник закрывается в обеих ветках: и когда сигнал взвели во время
 * итерации, и когда он был взведён до её начала. Иначе отвал клиента,
 * случившийся раньше первого элемента, оставлял бы источник открытым —
 * а именно так его читают транспорты.
 *
...
export async function* untilAborted<T>(
  ...
  if (signal.aborted) {
    // Итератор берётся ради немедленного закрытия: ресурс, занятый
    // источником в момент создания, освобождает именно `return()`
