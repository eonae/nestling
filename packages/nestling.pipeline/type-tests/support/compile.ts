/**
 * Общая оснастка компилятора для снапшот-тестов диагностик и для бюджета.
 *
 * Ключевое решение (design D7): над всем каталогом фикстур создаётся
 * **одна** программа. Программа на фикстуру стоит 2+ секунды каждая;
 * одна программа на каталог — единицы секунд на весь набор.
 */
...
export const typeTestsDir = resolve(here, '..');

/** Корень монорепозитория — база для нормализации путей в диагностиках */
export const repoRoot = resolve(typeTestsDir, '..', '..', '..');

export const fixturesDir = resolve(typeTestsDir, 'fixtures');
...
/**
 * Создаёт программу по tsconfig. Список файлов можно переопределить —
 * этим пользуется бюджет, меряя базу (пустой файл при том же tsconfig).
 */
export function createProgram(
  ...
  return ts.createProgram({
    rootNames: fileNames ?? parsed.fileNames,
