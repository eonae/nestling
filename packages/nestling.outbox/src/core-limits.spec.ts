 ...
 * операции, но `MetaOf<C>` даёт поле только команде. На приёме ключ лежит
 * в транспортном атрибуте, и подписчику события приходится доставать его
 * писателем `withIdempotencyKey()`.
 */
