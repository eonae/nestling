...
параметров: `UsersRepository$` для интерфейса `UsersRepository`, когда
DI-токен виден в файле, `Logger$.auto` для `Logger`, `AppConfig` для
`Config<typeof AppConfig>`, сам класс для класса. Напишите `@Handler()` с
пустым списком, и `eslint --fix` заполнит его. Тип, которого правило не
знает, оно принимает как написано, поэтому уровень правила `warn`.
