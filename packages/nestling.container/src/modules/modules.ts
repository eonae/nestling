...
export interface Module {
  ...
  /**
   * Умолчания модуля: провайдеры, которые попадают в граф, только если к
   * моменту `build()` под их токеном никто не зарегистрирован.
   *
   * Соперник может прийти из `providers` любого модуля, из фабрики
   * провайдеров или из `register()`, в любом порядке регистрации. Узел
   * умолчания атрибутируется этому модулю. Два умолчания под одним токеном
   * из разных модулей — ошибка регистрации.
   */
  defaults?: Provider[];
  /**
