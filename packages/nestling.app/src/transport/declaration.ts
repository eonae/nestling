...
export interface TransportDeclaration<Name extends string = string> {
   ...
   *
   * Дискриминатор нужен проверке `transports:`: поле принимает только
   * транспорты, и объявление сервера отвергается по нему на фазе BUILD.
   */
   ...
   *
   * Данные объявления, а не экземпляра: на фазе BUILD экземпляров нет,
   ...
   *
   * Единственный путь сервера в сборку. Транспорт, которому нужен сокет,
   * кладёт сюда объявление: либо переданное автором
   * (`http({ server: api })`), либо созданное фабрикой с тем же именем,
   * что у транспорта. Корень регистрирует его по этой ссылке, поэтому
 ...
 * них не знает. В `transports:` корня сервер не перечисляется — в сборку
 * он попадает по ссылке `server` того транспорта, который на нём работает.
 *
 ...
 *
 * Зовут его конструкторы транспортных пакетов (`server`), а не
 * прикладной код.
 *
 * @param declaration - Имя, DI-токен и провайдер экземпляра
 * @returns То же объявление, замороженное
