...
/**
 * DI-токен члена семейства: обычный DI-токен плюс два поля принадлежности.
 *
 * Семейство и параметр читаются полями. Разбора идентификатора нет:
 * DI-токен, чей `id` похож на `'Family:param'`, членом от этого не
 * становится.
 *
...
/**
 * DI-токен-заместитель «член по имени потребителя».
 *
...
/**
 * DI-токен-заместитель агрегата: все члены семейства одним массивом.
 *
...
/**
 * Семейство DI-токенов: один рецепт, много членов, различаемых параметром.
 *
 * Вызов семейства возвращает мемоизированный {@link FamilyMemberToken},
 * поэтому член годится везде, где нужен DI-токен: в `deps` у декоратора роли
 ...
 *
 * ILogger('users'); // DI-токен члена с id 'Logger:users'
 * ```
 */
...
> {
  /** Возвращает мемоизированный DI-токен члена для параметра. */
 ...
 *
 * Результат — функция: `Family(param)` возвращает DI-токен члена с `id`
 ...
 *
 * Членство читается полем `family` DI-токена. Похожий DI-токен, собранный
...
>(
  ...
  const family = (...params: Params): FamilyMemberToken<T> => {
    ...
    // члена собственный хэндл — так слой конфига в `@nestlingjs/app` вешает
...
/**
 * Возвращает семейство, которому принадлежит DI-токен-член.
 *
 * @param token - DI-токен
 * @returns Семейство либо `undefined`, если DI-токен членом не является
 */
...
/**
 * Заменяет DI-токен `.auto` на члена, названного по классу-потребителю.
 *
