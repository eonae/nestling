...
/**
 * Шов тестового корня, увиденный со стороны контейнера: подстановка узла,
 * прунинг осиротевшего поддерева и перечень недостающих зависимостей.
 *
 * Тесты намеренно смотрят на наблюдаемые следствия — какие конструкторы
 * выполнились, что осталось в `toJSON()`, в каком порядке отработали хуки, —
 * а не на внутренние структуры билдера.
 */
...
describe('overrides: подстановка узла графа', () => {
  ...
  it('делает фейк обычным узлом: его хуки идут в общем реверсе', async () => {
    const order: string[] = [];
  ...
  it('оставляет все узлы, их зависимости и порядок хуков', async () => {
    ...
    const DataModule = makeModule({
      ...
      providers: [
        ...
        // Узел, на который не ссылается никто: жадный контейнер обязан
        // оставить его в графе — прунинг не имеет права его «оптимизировать»
...
describe('прунинг: осиротевшие поддеревья', () => {
  ...
  it('оставляет членов семейства, нужных только через `.all`', async () => {
    ...
    const container = await new ContainerBuilder({
      ...
      // Единственный, кто называет членов поимённо, — заменяемый узел
      .register(
        factoryProvider(Repository, () => ({ find: () => 'real' }), [
          ISinkFamily('a'),
          ISinkFamily('b'),
  ...
  it('собирает агрегат после прунинга — по оставшимся членам', async () => {
    ...
    // Член 'b' нужен только заменяемому узлу и никакому `.all`-потребителю,
    // поэтому выпадает. Материализуйся агрегат **до** прунинга, его deps
    // указывали бы на выпавшего члена, и сборка упала бы на недостающем
    // провайдере вместо того, чтобы отдать агрегат из выживших.
    const container = await new ContainerBuilder({
      overrides: [[Repository, { find: () => 'fake' }]],
    })
      .register(
    ...
    // выживший потребитель агрегата не теряет ни одного из них
    expect(container.getOrThrow(Aggregated)).toEqual([
      { scope: 'b' },
      { scope: 'a' },
    ]);
