...
/**
 * Элемент списка `stubs:`: пара `DI-токен → значение` либо стаб операции.
 *
...
> {
   ...
   * позиционное: подменяется тот DI-токен, ссылка на который есть у теста;
   * строковой формы (`overrideByName('…')`) не существует.
   */
  overrides?: L;

  /**
   * Поставка недостающего: пары `DI-токен → значение`, регистрируемые
   * обычными провайдерами.
   *
   * Той же формы стаб операции: `stub(Operation, impl)` возвращает пару
   * `DI-токен вызывателя → фейк` и едет элементом этого же списка.
   */
...
export class TestApp {
   ...
   *
   * «Почему мой ресурс не захватился» отвечается данными, а не чтением
   * исходников.
   */
  ...
  /**
   * SHUTDOWN тестового прогона: взвод общего сигнала, затем `release`
   * ресурсов в обратном топологическом порядке. Идемпотентен.
   */
