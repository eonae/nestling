# logging-pino-package Specification

## Purpose

`@nestlingjs/logging.pino` — сателлит логирования: реализация `Logger`
поверх pino. Пакет отдаёт `pinoLogger(options)`; приложение передаёт её
корню опцией `logging: { logger }`. Формат `text` печатает общая функция
`formatLine` из `@nestlingjs/logging`, формат `json` пишет сам pino.
Зависимость у пакета одна — `@nestlingjs/logging`; сама библиотека стоит
в `peerDependencies`.
