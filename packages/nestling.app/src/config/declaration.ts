 ...
 * Порядок токенов семейства тот же, что у объявления: сначала поля рекорда, затем
 * вычисляемые.
 *
 ...
 *
 * У обычной секции этих токенов семейства нет — ни в типах, ни в рантайме.
 */
 ...
 *
 * Это сам токен семейства `ConfigSection`, на который дописан `.keys`:
 * инжект секции и упоминание токена семейства — одно и то же ребро графа, потому что
