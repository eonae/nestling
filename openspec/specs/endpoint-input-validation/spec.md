...
Вход endpoint'а проверяет рантайм пайплайна — не шаг и не транспорт. Точка
проверки одна: после всех `.pre`-шагов и до вызова хендлера, одинаково для
деклараций с полем `pipeline` и без него. Кандидат проверки — ключ `payload`
контекста, если его положил `.pre`-шаг, иначе `raw.payload`; что именно
...
`.pre`-шага. Ошибка конфигурации схемы отказом входа не становится: она
остаётся ошибкой приложения и даёт 500. Шага `validate()` в публичном API
...
состава его шагов. Точка проверки SHALL быть одна: после всех
`.pre`-шагов всех слоёв и до вызова хендлера. Хендлер SHALL получать
...
так же, как отказ `.pre`-шага: `.catch`-шаги всех слоёв применимы,
...
#### Scenario: Пайплайн без специального шага отвергает невалидный вход
...
#### Scenario: Отказ `.pre`-шага выполняется раньше проверки

- **WHEN** `.pre`-шаг авторизации бросает отказ с кодом `unauthorized`, а
  payload невалиден
...
`.pre`-шаг положил ключ `payload` в контекст, иначе — `ctx.raw.payload`.
...
#### Scenario: `.pre`-шаг подменяет кандидата

- **WHEN** `.pre`-шаг возвращает `{ payload: envelope.params }`, распаковав
...
- **WHEN** ни один `.pre`-шаг не кладёт `payload`
...
### Requirement: Шага `validate()` в публичном API нет

Пакет `@nestlingjs/app` SHALL NOT экспортировать шаг `validate()`.
