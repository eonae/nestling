...
рецептом семейства. Поэтому для застабанной операции боевой вызыватель
...
Матрица сводится объединением: операция, которая публикует только
топология `all`, в снапшоте есть и знает, кто его опубликовал. Иначе фича,
не выбранная в топологии `ops`, выглядела бы как удалённая операция.
