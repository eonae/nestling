...
Инжект массива вкладов, независимо зарегистрированных разными модулями
(health checks, миграции, валидаторы), поверх token families. `Family.all` —
сентинел-токен семейства, который на `build()` разворачивается в синтетический
узел-агрегат: его deps — токены всех зарегистрированных токенов семейства, его
значение — замороженный массив их инстансов. Вклад остаётся обычным провайдером
с токеном семейства; агрегат остаётся обычным узлом графа — циклы, топологические
...
Агрегат SHALL создаваться после фикспоинта создания токенов семейств и до
построения графа, регистрироваться обычным провайдером с deps = токены всех
зарегистрированных токенов семейства и значением — массивом их инстансов.
...
Состав агрегата SHALL включать всех токенов семейства, у которых на момент
создания агрегата есть провайдер: явно зарегистрированные вклады, токены семейства,
материализованные рецептом из deps, и токены семейства, полученные из `Family.auto`.
...
форсировать материализацию: токен семейства, созданный вызовом `Family(param)`, но не
упомянутый в deps и не имеющий явного провайдера, в массив SHALL NOT попадать.
Каждый токен семейства SHALL присутствовать в массиве ровно один раз, независимо от числа
...
- **WHEN** для семейства `IHealthCheck` не зарегистрирован `familyProvider`, но
  зарегистрированы два провайдера с токенами семейства, и потребитель объявляет
  `IHealthCheck.all`
...
- **THEN** токен семейства `"HealthCheck:db"` материализован рецептом один раз и входит в
  массив агрегата тем же инстансом, что инжектится первому классу

#### Scenario: Unreferenced member stays out of the aggregate

- **WHEN** вызван `IHealthCheck('orphan')` (токен создан), но этот токен семейства не
  упомянут в deps и не имеет явного провайдера, при этом потребитель объявляет
  `IHealthCheck.all`
- **THEN** массив агрегата не содержит токена семейства `'orphan'`, а узла
...
- **THEN** в графе один узел `"HealthCheck:{all}"`, оба класса получают один и
  ...
  токену семейства, представлен в массиве тем же единственным инстансом

### Requirement: Empty family aggregates to an empty array

`Family.all` семейства без единого зарегистрированного DI-токена SHALL давать узел
...
- **WHEN** ни одного токена семейства `IHealthCheck` не зарегистрировано и не
...
провайдеров-токенов семейства в билдере: явно зарегистрированные вклады — в порядке
регистрации модулей и провайдеров внутри модуля, затем токены семейства, материализованные
...
- **WHEN** зарегистрированы явный вклад `IHealthCheck('db')` и рецепт
  семейства, а токен семейства `IHealthCheck('redis')` материализуется из deps
  потребителя
...
детекция циклов (включая цикл `агрегат → токен семейства → агрегат`), топологический
...
- **WHEN** провайдер токена семейства `IHealthCheck('db')` объявляет dep
  `IHealthCheck.all`
...
- **THEN** `toJSON()`/`traverse()` содержат узел `"HealthCheck:{all}"` с
  рёбрами на узлы токенов семейства

### Requirement: Aggregate belongs to no module and consumes contributions as an outside consumer

Узел-агрегат SHALL иметь `metadata.module === undefined`. Его рёбра к токенам
семейства SHALL строиться без каких-либо ограничений со стороны модулей:
вклад объявляется провайдером токена семейства в `providers` модуля, и другого
