...
- **22 `contract-clients`** — **сделан**. После 11 (`makeContract`), 15
  ...
  zero-deps `@nestling/operations`, а `@nestling/ports` `makeContract` не
...
| 22 | `contract-clients` | M → **L** | **done** — [архив](../../openspec/changes/archive/2026-08-01-contract-clients/); `@nestling/operations` (переезд декларативного слоя, секция `http:`, контракт-форма `httpEndpoint`) + `@nestling/client`; оценка M не сошлась: цена не в новой логике, а в физическом переезде слоя между пакетами |
