...
| `UNLOCKED` | С влитого worktree снят лок. Проверь `ListAgents`; сессии нет — шаг «Уборка» |
...
5. **Уборка.** Признак, что worktree свободен, — отсутствие его сессии в
   `ListAgents` (по имени `change/X` или по полю `session`). Лок сам по
   себе ничего не значит: bridge держит его своим процессом и после
   закрытия сессии не снимает. Сессии нет — `git worktree unlock <path>`
   при необходимости, `git worktree remove <path>`, затем
   ...
   Когда убирать: сразу после проверки и потом, когда сессия закроется.
   Чтобы узнать о закрытии без опроса, вместе с финальным «всё» (шаг 6)
   подпишись: `SendMessage` с `notify_when_idle: true`. Первое
   уведомление придёт, когда сессия закончит отвечать пользователю, —
   подпишись ещё раз без текста; следующее придёт при выходе сессии, это
   и есть сигнал убирать. Событие `UNLOCKED` от скана — запасной путь на
   случай, если bridge всё же снял лок.
6. **Сессии change'а — «всё».** `SendMessage` той же сессии с
   `notify_when_idle: true`: change влит, проверки зелёные, работа
   закончена; пусть скажет пользователю одной строкой «change X влит в main
...
7. **Отчёт пользователю.** Одна строка в чате и `PushNotification` с тем же
   текстом:
