...
- **Пакетирование**: `makeContract`/`defineFail`, `Ok`/`Fail`, словарь
  статусов, формы io и bind-карта живут в `@nestling/contracts` — zero
  runtime deps (транзитивно только types-only Standard Schema) ⇒ контракт
  импортируем во фронт и скрипты. Инвариант проверяется тестом, обходящим
  граф импортов собранного пакета: обещание не должно зависеть от
  tree-shaking в инструменте потребителя. Примитив токена инжекции
  приезжает subpath'ом `@nestling/container/tokens` (листовые модули без
  runtime-импортов) — иначе `.port`/`.emitter` не были бы настоящими
  членами семейств. Рантаймовая часть — вызыватели, шина и биндинг — в
  `@nestling/ports`; он реэкспортирует **типы вызывателей**
  (`Port`/`Emitter`/`PortMeta`), но **не** `makeContract`: этот реэкспорт
  вернул бы объявление контракта в пакет с серверными зависимостями и
  сделал бы упаковочную гарантию вопросом дисциплины импортов.
...
Контракт с секцией `http:` реализуется на HTTP той же дисциплиной —
**контракт-формой** `httpEndpoint({ contract, deps?, pipeline?, handle,
detached? })`: `method`/`path`/`bind`/`rawBody`/`sse`/`input`/`output`/
`errors` объявлены в её словаре как `never`, bind-карта берётся с контракта
**тем же значением** и повторно не считается. Без серверной формы `http:`
описывал бы адрес, который никто не обязан обслуживать: автор писал бы
`httpEndpoint({ method, path, input, … })` руками и рано или поздно
разъехался бы с контрактом — ровно тот разъезд двух артефактов, из-за
которого в d/07 отвергнут `httpBind()`. Контракт без `http:` отвергается в
момент создания декларации; текст предлагает объявить секцию либо
реализовать контракт на шине через `implement`.
...
- **Валидация ответа — по умолчанию**, opt-out явный
  (`makeClient(record, { validateOutput: false })`): обещание клиента —
  типизированный результат, и отдать невалидированный JSON под типом
  `Output` значит соврать в типе.
- **Коерсия query закрыта и не догадывается**: `undefined`/`null` не
  пишутся, скаляр — `String(value)`, массив скаляров — повторяющиеся
  вхождения ключа, всё остальное — `TypeError` в момент вызова с именем
  поля. Молчаливого `[object Object]` на проводе не бывает. Обратная
  сторона: схема query-поля обязана принимать проводную форму
  (`z.stringbool()`, `z.coerce.*`) — query несёт строки.
- `meta` клиента — `{ signal?, deadline? }`. `deadline` (абсолютный момент,
  как у портов) проверяется **до** отправки: истёкший бюджет даёт
  `DeadlineExceeded` без похода в сеть. `idempotencyKey` в v1 нет —
  согласованного HTTP-слота для него не существует.
- Клиент **не бросает** на транспортных и контрактных сбоях у `request` —
  возвращает `Fail`. Бросает только на дефектах использования.
- `makeClient` отвергает **в момент создания** (не при первом вызове):
  контракт без `http:`, вид `event`, потоковую или `multipart`-форму io,
  не-JSON тело, неабсолютный `baseUrl` — называя ключ метода в record'е.
- `makeClient` живёт в `@nestling/client`: fetch-based, без Node-специфики;
  граница пакета проверяется тем же обходом графа импортов.
