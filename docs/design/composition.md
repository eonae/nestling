...
`[RequestId, Trace]`. Сборка оборачивает переданный логгер декоратором
...
`stdout` занят результатом команды.
...
([testing.md](./testing.md)). `@nestlingjs/logging.pino` даёт `pinoLogger(options)` для опции
`logger`: пишет в `stderr`, человекочитаемый формат без `pino-pretty`.
