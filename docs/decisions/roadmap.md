...
| 12 | `transport.nats` | NATS как inbound+outbound транспорт; queue-groups для реплик; remote-биндинг портов; JetStream для `durable` (split, L4) | M | **done** — [ideas.md [2026-07-31] «NATS: шина приложения»](./ideas.md) |
...
| 12 | `transport.nats` | M | **done** — remote-биндинг, queue-groups, JetStream, wire-часть `propagate` из 16; пакет `@nestling/transport.nats` и пример `examples.split-nats` |
