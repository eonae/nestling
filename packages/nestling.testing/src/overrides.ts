/**
 * Подстановки тестового корня: пары `DI-токен → фейк` и подмена рецепта
 ...
 * DI-токена: подмена не того типа — ошибка компиляции, а не рантайм-сюрприз.
...
/** Значение — подмена рецепта семейства, а не пара `DI-токен → фейк`? */
