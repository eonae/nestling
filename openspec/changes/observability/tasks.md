...
- [ ] 7.1 Прогнать бенч под Node 24 до правок и записать результат
- [ ] 7.2 Прогнать бенч после правок на приложении без опции `metrics`:
  расхождение с бюджетом `hot-path-trim` объяснено или отсутствует
- [ ] 7.3 Прогнать `yarn type-budget`: бюджет типов не сдвинут

## 8. Пример `split-nats`

- [ ] 8.1 Добавить `withTracing()` в базовый слой примера
- [ ] 8.2 Написать адаптер `Metrics` с форматом Prometheus и endpoint
  `/metrics` внутри примера, подключить его через `makeApp({ metrics })`
- [ ] 8.3 Тест примера: записи двух процессов несут один `traceId`
- [ ] 8.4 Проверить, нужен ли `withTracing()` примерам `users-service` и
