...
Клиента Vault пакет не берёт: запрос идёт штатным `fetch`, поэтому в
зависимостях только ядро и `zod`.
...
AWS в пакет не входят: минимальный источник ходит с готовым токеном, а
способ его получить остаётся за стендом.
