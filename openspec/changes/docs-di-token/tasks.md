...
- [x] 4.4 Прогнать `yarn docs:build`: сборка одного файла не падает,
...
- [x] 12.4 `design/` и `decisions/` синхронизированы по правилам
      `CLAUDE.md`
- [x] 12.5 `yarn docs:audit` — 0 ERROR
- [x] 12.6 Затронутые `examples/*` мигрированы; главы гайда пересверены, и
      если сессия идёт позже 2026-09-06 — дата в плашке «сверено с кодом»
      обновлена
- [x] 12.7 Коммиты осмысленные, ветка `change/docs-di-token` запушена
