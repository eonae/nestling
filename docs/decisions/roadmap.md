...
| 17 | `pipeline-drop-after` | убрать `.after` из билдера/типов/рантайма (`ResponsePhase` = `'ok' \| 'catch'`); словарь ответного тракта — Promise-тройка `ok`/`catch`/`finally`; правка спеков и доков (`docs/preview`) | S, breaking | **реализован** — ждёт архивации, [ideas.md [2026-07-10]](./ideas.md) |
...
| 17 | `pipeline-drop-after` | S | чистое удаление фазы; после появления новых слоёв дорожает — **реализован**, ждёт архивации |
| 8 + 24 | `endpoint-discovery` + `endpoint-model` | S + M | одним заходом: глобальный реестр умирает вместе с `@Endpoint` |
| 21 | `input-bind` | M | поверх новых деклараций; несущий уровень для 20 и 22 |
| 23 | `pipeline-type-dx` | S–M | закрывает окно: снапшоты диагностик и порог tsserver снимаются с уже зафиксированных типов |
...
| # | Change | Размер | Почему здесь |
|---|---|---|---|
| 15 | `error-model` | M | несёт баг-фикс: возвращённый `Fail` сейчас уезжает как `200 OK` |
...
| # | Change | Размер | Почему здесь |
|---|---|---|---|
| 11 | `ports` | L | `makeContract`, `Port`/`Emitter`, `InProcessBus`, dispatch-политики |
