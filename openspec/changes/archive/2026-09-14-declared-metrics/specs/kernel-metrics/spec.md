...
`makeMetrics('orders', { created: counter({ … }) })` и подключения её
вкладом `metrics:`.

### Requirement: `RootMetrics$` и семейство `Metrics$`

**Reason**: Накопленное держит ядро (`MetricsStore$`, capability
`metrics-store`), поэтому корня, который задаёт чужую реализацию записи,
больше нет. Область записи переехала из атрибута `scope` в префикс имени
метрики.

**Migration**: Опция `makeApp({ metrics })` удаляется из корня.
Потребители `Metrics$.auto` и `Metrics$(scope)` переходят на группу своей
фичи. Экспорт подключается плагином, читающим `MetricsStore$`.

### Requirement: Приложение без метрик не платит за них

**Reason**: Платить стало не за что: запись объявленного ряда — прибавка по
индексу, вычисленному на сборке. Выключатель через сравнение корня с пустой
реализацией снят вместе с корнем.

**Migration**: Ничего делать не нужно. Приложения, полагавшиеся на
отсутствие опции `metrics` как на выключение, получают накопление в store;
наружу оно не уходит, пока не подключён плагин экспорта.

### Requirement: `spyMetrics()` — записи значениями

**Reason**: Перехватывать нечего: записи лежат в store тестового
приложения, и тест читает снимок.

**Migration**: `spyMetrics()` и `overrides: [[RootMetrics$, spy.metrics]]`
удаляются из тестов; утверждения переписываются на чтение снимка
