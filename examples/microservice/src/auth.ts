...
/**
 * Pre-шаг: проверяет Bearer-токен и кладёт `caller` в контекст.
 *
 * Bearer-токен сравнивается со значением из секции конфига. Если значения
 * не совпали, шаг возвращает отказ: ни следующие шаги, ни хендлер не
 * вызываются.
 */
@Handler([AppConfig])
 ...
 *
 * `Unauthorized` объявлен здесь, при подключении шага: endpoint'ы со
