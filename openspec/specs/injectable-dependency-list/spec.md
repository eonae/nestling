...
Список токенов в декораторе роли сверяется с параметрами роли целиком:
по типам, по порядку и по длине. Эталон параметров задаёт роль: у
`@Component` и `@Handler` — конструктор, у `@Resource` — `static acquire`
без последнего параметра. Длиной списка распоряжается компилятор —
допустимая длина берётся из параметров эталона, поэтому необязательные
параметры дают диапазон длин, а rest-параметр снимает верхнюю границу. Слишком длинный список отвергается литералом `__error` с
