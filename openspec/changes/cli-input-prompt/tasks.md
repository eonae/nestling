...
- [ ] 2.1 `CliEndpointDictionary`: поле `missing?: 'error' | 'prompt'`,
      JSDoc вместо комментария-отсылки к отдельной работе
- [ ] 2.2 `cliEndpoint`: снять `missing` со словаря и положить в
      `binding: { missing }`; политика `'error'` поля `binding` не добавляет
- [ ] 2.3 Чтение политики с проекции маршрута: функция уровня модуля по
      образцу `httpBindingOf`
- [ ] 2.4 Спека: политика видна транспорту, декларация исполнима без
      изменений в ядре

## 3. Потоки транспорта

- [ ] 3.1 `CliTransportOptions`: `input`, `output`, `errorOutput` с
      умолчаниями `process.stdin`, `process.stdout`, `process.stderr`
- [ ] 3.2 Перевести печать результата, отказа и NDJSON с `console` на
      `output`/`errorOutput`; снять `eslint-disable no-console`
- [ ] 3.3 Чтение формы `stream(...)` — из `input`, а не из `process.stdin`
- [ ] 3.4 REPL: интерфейс readline поверх `input`/`output`
- [ ] 3.5 Спеки: результат уходит в подставленный поток, поток команды
      читается из подставленного ввода

## 4. План вопросов

- [ ] 4.1 `CliTransportOptions.converters` и `interactive`
- [ ] 4.2 Построение плана на `serve`: JSON Schema формы `input` в
      направлении `io: 'input'`, перечень полей из `required`, форма
      вопроса по узлу
- [ ] 4.3 Ошибка `serve` при отсутствии конвертера: имя команды, вендор,
      опция `converters`
- [ ] 4.4 Ошибка `serve` при `stream`-входе вместе с `missing: 'prompt'`
- [ ] 4.5 Узел непонятной формы пропускается без вопроса и без ошибки
- [ ] 4.6 Спеки: план строится один раз, конвертер нужен только командам с
      политикой

## 5. Вопросы в терминале

- [ ] 5.1 Определение интерактивности: `input.isTTY` и переменная `CI`,
      опция `interactive` перекрывает
- [ ] 5.2 Формы вопроса: нумерованный список для `enum`, `[y/n]` для
      `boolean`, строка для скаляра; `description` подсказкой, `default` по
      пустому вводу
- [ ] 5.3 Форма ответа: строка, `true`/`false`, элемент списка; приведения
      к типу JSON Schema нет
- [ ] 5.4 Один интерфейс readline: в `'repl'` — открытый, в `'argv'` —
      временный на время вопросов
- [ ] 5.5 Конец ввода оставляет поле незаданным
- [ ] 5.6 Достроенный payload уходит в `dispatch.call` тем же путём, что и
      собранный из argv

## 6. Тесты транспорта

- [ ] 6.1 Команда без политики: вопросов нет, отказ `bad_request`
- [ ] 6.2 Выбор из списка, подтверждение, подсказка с умолчанием
- [ ] 6.3 Заданное флагом поле не спрашивается
- [ ] 6.4 Число вводится строкой и приводится схемой
- [ ] 6.5 Отключение вопросов переменной `CI` и опцией `interactive`
- [ ] 6.6 Вопрос внутри REPL: ответ не разбирается как команда
- [ ] 6.7 Ошибки `serve`: нет конвертера, `stream` вместе с политикой
