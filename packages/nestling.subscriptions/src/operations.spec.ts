/**
 * Факты жизненного цикла: схемы пакета и поведение реестра при отказе
 * публикации.
 *
 * Проверка схем здесь не про валидацию вообще, а про два утверждения:
 * факт написан на вендоре фреймворка, поэтому переводится штатным
 * конвертером, а приложение при этом видит нейтральный тип.
 */
...
describe('схемы фактов', () => {
  it('написаны на вендоре фреймворка: их переводит штатный конвертер', () => {
    expect(
