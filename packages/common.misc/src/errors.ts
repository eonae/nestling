 ...
 *
 * Спека допускает сегмент-объект `{ key }` и ключ-символ; `issues` попадают
 * в тело HTTP-ответа, поэтому их JSON-форма не должна зависеть от того,
 * как вендор упаковал путь.
 */
