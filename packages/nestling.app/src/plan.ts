 ...
 *
 * Модуль **не** реэкспортируется из `index.ts`: наружу уезжает только
...
export interface TestSubstitutions {
  ...
  /** Подмены рецептов семейств — до материализации членов */
...
/** Ручка, погашенная фазой WIRE: исходная декларация и всё, чем её исполнить */
export interface WiredEndpoint {
  ...
  /** Её исполнимая копия (зависимости погашены контейнером) */
  readonly executable: ExecutableDeclaration;

  /** Диспетчер транспорта этой ручки */
  readonly dispatch: Dispatch;

  /** Модуль, объявивший ручку */
 ...
 *
 * `dispatch` рождён, START не выполнялся: транспорты не в эфире,
 * обработчики сигналов процесса не поставлены, строка состава не напечатана.
 */
export interface WiredApp {
  /** Собранный граф: `@OnInit` выполнены, `@OnStart` — нет */
  readonly container: BuiltContainer;

  /** Ручки приложения, адресуемые по идентичности их деклараций */
  readonly endpoints: ReadonlyMap<AnyEndpointDefinition, WiredEndpoint>;

  /** Выбранные фичи — то же, что увидел бы `run()` */
  readonly features: readonly Feature[];

  /** Общий сигнал прогона: уезжает в каждый `call`, взводится на `close()` */
