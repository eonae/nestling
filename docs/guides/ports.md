...
токену, а вызывает его через операция: описание операции с именем, схемами
...
Вызов через операция с самого начала асинхронный, возвращает ошибки
...
Версия операции входит в его имя: `users.create.v2`. Отдельного поля
...
у них владелец ровно один, и вторая реализация того же операции — ошибка
...
вызывателя создаётся при сборке для каждого операции, который кто-то
упомянул в `deps`.
...
- `snapshotOperations` объединяет отчёты всех топологий. Операция, который
  ...
  называет эту топологию. Так «фича не выбрана» и «операция удалён» не
  путаются.
