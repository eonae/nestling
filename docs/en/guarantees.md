...
| A failure of a pre-unit is declared on the layer | at compile time | [10](./guide/10-auth.md) |
...
| All dependencies are registered, the graph has no cycles | phase ASSEMBLE | [6](./guide/06-repository.md) |
| Every endpoint has connected the required layer | phase ASSEMBLE | [10](./guide/10-auth.md) |
| A mutating endpoint is composed from the transaction layer | phase ASSEMBLE | [11](./guide/11-database.md) |
| A bus subscriber is composed from the inbox layer | phase ASSEMBLE | [16](./guide/16-durable-events.md) |
| The pipeline has declared a variable that is read from deep in the graph | phase ASSEMBLE | [10](./guide/10-auth.md) |
| The io shapes of the declarations are supported by the transport | phase ASSEMBLE | [12](./guide/12-files-and-streams.md) |
| Every schema has a converter to OpenAPI | phase ASSEMBLE | [13](./guide/13-openapi-and-client.md) |
| There is no direct graph edge between two features | phase ASSEMBLE | [14](./guide/14-features.md) |
| Every called operation has an owner or a bus | phase ASSEMBLE | [20](./guide/20-split.md) |
...
The checks of the ASSEMBLE phase run before the instances are created
...
and by the test assembly `assembleTest`
