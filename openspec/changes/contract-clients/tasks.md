...
- [ ] 4.1 Добавить в `ContractSpec` поле `http` в двух формах (строка и
      запись); строгий разбор строковой формы с fail-fast
- [ ] 4.2 Вызвать `computeHttpBinding` в `makeContract`, положить карту на
      значение контракта; типизировать ключи `bind` полями `input` за вычетом
      path-параметров (как в HTTP-словаре)
- [ ] 4.3 Рантайм-тесты fail-fast: битая строковая форма, пустой путь и путь
