...
Реализация операции («я обрабатываю `billing.charge`») — это endpoint на транспорте-шине. Единственное новое — типизированный вызыватель. Владелец реализует операция, потребитель инжектирует `Operation.caller`:
