...
> 🛰️ **Zod-специфичный сателлит вне ядра V1.** Ядро Nestling принимает любую
> [Standard Schema](https://standardschema.dev) (zod, valibot, arktype, …) и
> схемы не интроспектирует. Этот пакет намеренно делает обратное: его
> центральная фича — сверка схемы с уже существующим доменным типом по полям —
> требует zod-нативной интроспекции (`z.ZodObject<infer Shape>`, `z.input<S>`),
> которой спека не даёт. Отсюда собственный явный `peerDependencies.zod`;
> у ядра его нет.
