...
- [x] 13.1 Все задачи `tasks.md` отмечены
- [x] 13.2 `yarn verify` зелёный (`build` + `lint` + `test` по всем пакетам)
- [x] 13.3 README затронутых пакетов обновлены, включая плашки статуса
- [x] 13.4 `design/` и `decisions/` синхронизированы по правилам `CLAUDE.md`
- [x] 13.5 `yarn docs:audit` — 0 ERROR
- [x] 13.6 Затронутые `packages/examples.*` мигрированы, гайды пересверены с
      обновлённой датой в плашке «сверено с кодом»
- [ ] 13.7 Коммиты осмысленные, ветка запушена — *коммиты готовы (7 шт.);
