...
| 55 | `stream-finish` | `.finally` у потокового ответа, закрытого до первого элемента: обёртка `withFinish` — асинхронный генератор, а `return()` на неначатом генераторе тело не исполняет, поэтому отложенные юниты не отрабатывают и запись подписки живёт до конца процесса; контракт `design/transports.md` «потребить итератор либо закрыть его» выполняется наполовину | S | план — находка №4 замера [ideas.md [2026-08-01]](./ideas.md) «Реестр подписок»; поведение держит тест `core-limits.spec.ts`, починка уронит его и заставит обновить запись |
...
| 58 | `docs-package-readmes` | волна 2 уборки: шаблон README пакета и переписывание шести больших — `app`, `container`, `transport.http`, `operations`, `testing`, `models` (запись называла `pipeline` и `ports`, влитые в `app` change'ем 42) | M | план — [ideas.md [2026-09-05]](./ideas.md) «Структура документации»; после 40 |
...
| 55 | `stream-finish` | S | контракт с транспортом обещает «потребить итератор либо закрыть его», и вторая половина не работает: подписка, закрытая до первого элемента, остаётся в реестре до конца жизни процесса |
...
| 58 | `docs-package-readmes` | M | README пакетов пишутся каждый по-своему, а шести большим нужен не шаблон, а переписывание |
...
`docs-layout-cleanup` (40), остальные четыре стоят выше строками 58–61.
