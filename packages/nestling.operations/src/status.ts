 ...
 *
 * Оба перечня закрыты и не зависят от транспорта. Код для сети выбирает
 * транспорт: HTTP берёт число из своей таблицы, шина несёт статус как
 * есть.
 */
 ...
 * зовут там, где код уже принят (`makeFail`) или пришёл по сети.
 */
