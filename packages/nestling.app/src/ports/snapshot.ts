...
/**
 * Отчёт структурной проверки — **структурно**, без импорта из каталога
 * `root`.
 *
 * Стрелка зависимостей идёт от корня к слою портов, и обратный импорт
 * запрещён правилом зон. Сведению нужно ровно поле `published`, и оно его
 * и требует.
 */
