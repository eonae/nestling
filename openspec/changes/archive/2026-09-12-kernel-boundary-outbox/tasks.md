## 1. Подсказки DI-токенов (`@nestlingjs/container`)

- [x] 1.1 `src/common.ts`: `makeToken<T>(id, options?)` принимает
  `{ hint }`; значение остаётся замороженным, идентичность DI-токена
  ссылочная. JSDoc говорит, что подсказка — текст починки, а не описание
- [x] 1.2 `src/builder/container.builder.ts`,
  `assertDependenciesSatisfied`: под строкой перечня печатаются
  подсказки — недостающего DI-токена и каждого потребителя, у которого
  она объявлена, по строке на DI-токен с его идентификатором
- [x] 1.3 Формат для DI-токенов без подсказок не меняется: перечень,
  потребители и завершающая строка про `providers:` остаются прежними
- [x] 1.4 Рантайм-тесты в `src/builder/container.builder.spec.ts`:
  подсказка недостающего, подсказка потребителя, обе сразу, ни одной;
  три дыры перечисляются по-прежнему
- [x] 1.5 Существующие спеки формата ошибки (`token-families.spec.ts`,
  `overrides-pruning.spec.ts`, `instantiation.spec.ts`,
  `@nestlingjs/testing` `unit.spec.ts`) прогнаны без правок

## 2. Словарь `meta` вызова (`@nestlingjs/operations`)

- [x] 2.1 `src/families.ts`: `CommandMeta` переименован в `EmitMeta`;
  `MetaOf<C>` выбирает `PortMeta` по дискриминанту `kind: 'request'` и
  `EmitMeta` в остальных случаях. Имени `CommandMeta` не остаётся ни в
  одном пакете
- [x] 2.2 `InvokeArgs<C, M = MetaOf<C>>`, `Port<C, M = MetaOf<C>>` и
  `Emitter<C, M = MetaOf<C>>`; рантайм не меняется
- [x] 2.3 Тип-тесты: `emit(payload, { idempotencyKey })` компилируется у
  события и не компилируется у `request`; расширенный
  `Emitter<C, MetaOf<C> & { partitionKey?: string }>` присваивается
  `Emitter<C>`; payload остаётся обязательным там, где был
- [x] 2.4 `yarn type-budget` замерен до и после правки `families.ts`:
  инстанциации 119 753 → 120 293 (+0.5 %), типы 33 633 → 33 746 (+0.3 %),
  hover 140 → 139 мс. Пороги не тронуты, тип-параметр остаётся у обоих —
  и у `Port`, и у `Emitter`

## 3. Ключ идемпотентности события (`@nestlingjs/app`)

- [x] 3.1 `src/ports/invoker.ts`, `idempotencyKeyOf`: команда получает
  переданный либо чеканенный ключ, событие — только переданный;
  `request` ключа не имеет. Обе ветки — и локальный эмиттер, и
  remote-эмиттер
- [x] 3.2 JSDoc, где ключ назван «командой»: `PublishOptions`,
  `BusMessageMeta`, `Envelope` в `src/ports/bus.ts` и
  `profileAttributes` в `src/ports/profile.ts` — текст приведён к «без
  ответа», конверт не меняется
- [x] 3.3 Рантайм-тесты в `src/ports/bus.spec.ts` и
  `src/ports/profile.spec.ts`: событие с переданным ключом приходит
  подписчику на обоих путях биндинга; событие без ключа едет без него;
  команда без ключа по-прежнему получает чеканенный
- [x] 3.4 `withIdempotencyKey()` не меняется: подписчик события без
  присланного ключа получает собственный — тест это фиксирует

## 4. Писатель переменной с зависимостями (`@nestlingjs/app`)

- [x] 4.1 `src/pipeline/core/context/variable.ts`: вторая форма
  `provide(deps, compute)`. Юнит несёт прежнюю неперечислимую пометку
  переменной и вторую — список DI-токенов; `compute` получает контекст
  первым параметром и значения зависимостей следом
- [x] 4.2 `src/pipeline/core/pipeline.ts`: `UnitResolver` принимает
  `InjectionToken`, запись юнита несёт список DI-токенов, `bind()`
  резолвит их один раз и подставляет замыкание. Пометка переменной
  читается `declaredVarOf` так же, как у юнита без зависимостей
- [x] 4.3 `TNeeds` пополняется DI-токенами писателя: пайплайн с таким
  писателем не принимается транспортом до `bind()`
- [x] 4.4 `Family.auto` в списке зависимостей отвергается ошибкой,
  называющей переменную и предлагающей явный вызов семейства
- [x] 4.5 Тип-тесты: вывод типа зависимости из списка DI-токенов;
  требования из аннотации `ctx`; непустой `TNeeds` до `bind()`. Живут там
  же, где остальные тип-тесты пайплайна, — в `pipeline.spec.ts` и
  `context/context.spec.ts` (`TYPE-TESTS.md`). Вывод сработал: `db`
  типизируется из списка, `TReq` — из аннотации `ctx`, запасной вариант
  не понадобился
- [x] 4.6 Рантайм-тесты пайплайна: писатель кладёт значение, полученное
  из контейнера; резолв случается один раз на `bind()`, а не на запрос;
  `hasVar` засчитывает такого писателя
- [x] 4.7 Тест сборки в `@nestlingjs/app`: endpoint с писателем, чья
  зависимость не зарегистрирована, роняет ASSEMBLE с паттерном,
  модулем-объявителем и недостающей зависимостью

## 5. `@nestlingjs/outbox` переезжает на новые примитивы

- [x] 5.1 `src/emitter.ts`: тип `OutboxEmitMeta<C>` и `OutboxEmitter<C>`;
  `emit` читает `partitionKey` из `meta`. Тип `PartitionKeyOf` и поле
  `partitionKey` в `OutboxEmitterContext` удалены
- [x] 5.2 `src/plugin.ts`: опция `partitionKey` у `outbox({ … })`
  удалена вместе с её проверкой в `assertOptions`
- [x] 5.3 `src/plugin.ts`: идентификатор `OutboxBus$` становится именем
  (`'@nestlingjs/outbox relay'`), починка переезжает в `hint`
- [x] 5.4 `src/core-limits.spec.ts` удалён: все четыре находки закрыты
- [x] 5.5 `src/emitter.spec.ts` и `src/plugin.spec.ts`: раздел из места
  вызова, запись без раздела, текст ошибки без шины с обеими подсказками
- [x] 5.6 `src/index.ts`: `OutboxEmitter` экспортируется, `PartitionKeyOf`
  — нет

## 6. Пример `examples/users-service`

- [x] 6.1 `src/persistence.ts`: класс `ProvideDb` удалён из кода и из
  `providers:`, слой транзакции состоит из одного писателя
  `Tx.provide([Database], (_ctx, db) => db.begin())`
- [x] 6.2 `src/app.ts`: опция `partitionKey` убрана из `outbox({ … })`
- [x] 6.3 `src/users/endpoints/create-user.endpoint.ts`: раздел передаётся
  вызовом `emit(user, { partitionKey: user.id })`
- [x] 6.4 `src/app.spec.ts` зелёный; `yarn smoke` проходит

## 7. Документация

- [x] 7.1 `docs/design/pipeline.md`: вторая форма писателя переменной и
  резолв его зависимостей на WIRE; правило «юнит с зависимостями — класс,
  кроме писателя переменной»
- [x] 7.2 `docs/design/operations.md`: ключ идемпотентности у события,
  чеканка только у команды, словарь `meta` как точка расширения
- [x] 7.3 `docs/design/container.md`: подсказка на объявлении DI-токена и
  её место в тексте ошибки сборки
- [x] 7.4 README `@nestlingjs/app`, `@nestlingjs/container`,
  `@nestlingjs/operations` и `@nestlingjs/outbox` сверены, включая плашки
  статуса; из README outbox'а уходит абзац про юнит-мост
- [x] 7.5 Главы гайда 14 и 27 перечитаны и правлены: слой транзакции в
  один юнит, раздел записи в месте вызова, ключ идемпотентности события;
  дата в плашке «сверено с кодом» обновлена
- [x] 7.6 `docs/decisions/ideas.md`, запись [2026-09-07] «Транзакционный
  outbox»: каждая из четырёх находок раздела «Результат замера границы»
  несёт пометку «РЕАЛИЗОВАНО» с тем, что вышло целиком, что уехало дальше
  и чем реализация уточнила решение
- [x] 7.7 `docs/decisions/roadmap.md`, строка 57: описание приведено к
  итоговому решению
- [x] 7.8 Purpose спек `port-idempotency` и `transactional-outbox`
  правится вручную при вливании дельт: формулировки «только у `command`»
  и «ядро ради неё не меняется» устарели

## 8. Definition of Done

- [x] 8.1 Все задачи выше отмечены
- [x] 8.2 `yarn verify` зелёный (build, typecheck, lint, test, type-budget)
- [x] 8.3 README затронутых пакетов обновлены, включая плашки статуса
- [x] 8.4 `design/` и `decisions/` синхронизированы по правилам `CLAUDE.md`
- [x] 8.5 Запись `ideas.md`, по которой шёл change, несёт пометку
  «РЕАЛИЗОВАНО»
- [x] 8.6 `yarn docs:audit` — 0 ERROR
- [x] 8.7 Затронутые `examples/*` мигрированы, главы гайда пересверены с
  обновлённой датой в плашке «сверено с кодом»
- [x] 8.8 Коммиты осмысленные, ветка запушена
