...
- [x] 2.3 `tap(sink)`: стартовое состояние при подписке, поток записей,
  функция отписки, изоляция исключения получателя с записью в логгер ядра
- [x] 2.4 `MetricSink` и `MetricAttributes` в барели пакета; интерфейс
  `Metrics` удалён
...
- [x] 6.4 `e2e` примеров проходят; экспозиция свежеподнятого примера
...
- [x] 8.1 `docs/decisions/roadmap.md`: строка на этот change со ссылкой на
