...
### Requirement: Читалка трассы для кода вне ядра

`@nestlingjs/app` SHALL экспортировать `traceparent(): string | undefined`
— трассу текущего запроса строкой W3C trace-context или `undefined`, если
трассы нет.

Идентификатор участка в строке SHALL быть собственным: получатель
становится ребёнком того участка, который отправляет запрос.

Читалка существует потому, что её потребитель живёт вне пакета:
типизированный HTTP-клиент зависит только от `@nestlingjs/operations` и
собирается для браузера, поэтому ambient-контекст ему недоступен. Клиент
принимает эту функцию опцией `trace` (capability `typed-http-client`).

#### Scenario: Строка внутри трассируемого запроса

- **WHEN** `traceparent()` вызвана из обработчика, чей пайплайн содержит
  `withTracing()`
- **THEN** возвращается `00-<traceId>-<spanId>-01` этого запроса

#### Scenario: Вне запроса строки нет

- **WHEN** `traceparent()` вызвана из `@OnStart`
