...
Компилятор сверяет типы и порядок. Аргумент другого типа, перепутанный
порядок токенов и список короче, чем список параметров, дают ошибку
компиляции. Лишний токен, которого конструктор не принимает, компилятор
пропускает: функция с меньшим числом параметров подходит под тип функции
с большим.
...
`@Injectable` класса и поля `handler`. Инжектировать при этом можно
