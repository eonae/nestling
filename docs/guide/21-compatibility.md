...
Schema конвертер вендора — тот же `zodConverter`, который генератор
документа из главы [13](./13-openapi-and-client.md) подставляет
умолчанием. Здесь умолчания нет: `checkTopologies` принимает список явно,
потому что структурная проверка живёт в ядре, а конвертер — снаружи. Без
конвертера лист помечается непрозрачным, и сравнение по нему даёт вердикт
`unknown`.
...
[рецептами](../recipes/README.md) — например, [webhook с проверкой
подписи](../recipes/webhook.md).
