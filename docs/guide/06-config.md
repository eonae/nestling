...
Порт, адрес базы и DI-токен API должны приходить из переменных окружения.
...
| `API_TOKEN` | `apiToken` | DI-токен для запросов, меняющих данные | нет, переменная обязательна |
