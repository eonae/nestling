...
- [x] 6.2 `yarn verify` зелёный (`build` + `typecheck` + `lint` + `test` + `type-budget` по всем пакетам)
...
- [x] 6.4 `design/` и `decisions/` синхронизированы: упаковку они не описывают, правило про исключение тестового кода из сборки живёт в CLAUDE.md и обновлено там
...
- [ ] 6.7 Коммиты осмысленные, ветка запушена — четыре коммита на месте, пуш за пользователем: SSH-ключ заперт в keychain, из сессии агент пуст
