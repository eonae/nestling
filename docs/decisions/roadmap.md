...
| 66 | `drizzle-pg` | L | база данных — первая инфраструктура каждого сервиса, а Nestling не давал для неё ничего: пример держал таблицу в памяти, слой транзакции писался руками, адаптер `OutboxStore` был работой приложения; решение — [ideas.md [2026-09-11]](./ideas.md) «Соединение с базой: сателлит `drizzle.pg`» |
