...
- **Переменная участка** — `Span`, `OtelSpan`: `Ctx(Span)` даёт сервису
  дописать атрибут или событие на текущем участке.
...
Имя участка — шаблон маршрута из декларации, атрибуты — `transport`,
`pattern` и `outcome`. Статус выводится из исхода: `completed` даёт `OK`,
`failed` и `aborted` — `ERROR`, `disconnected` остаётся `UNSET`. Уход
клиента не отказ сервиса, и доля ошибок в дашборде не растёт от поведения
клиентов.
...
поэтому записи последнего интервала не теряются.
...
`@nestlingjs/prometheus` показывают одно и то же; оба плагина читают один
store и друг о друге не знают.
