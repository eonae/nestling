 ...
 * из юнита, хендлера, ответного тракта и тела потока. Ридер здесь
...
describe('проекция догоняет pre-тракт', () => {
...
describe('ответный тракт под тем же scope', () => {
  ...
  it('на упавшем pre-тракте peek() — undefined, а get() бросает', async () => {
