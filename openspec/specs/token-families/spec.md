...
токенов семейства, что упомянуты в deps зарегистрированных провайдеров. Материализованный
токен семейства неотличим от обычного узла графа — дедупликация, циклы, lifecycle,
атрибуция к модулю и визуализация работают без исключений. Рантайм-резолюции
токенов семейства не существует.
...
- **WHEN** то же приложение собрано через `buildTest` с подменой
...
Токен семейства: объект с `id` вида `"<name>:<param>"` и полями принадлежности
`family` и `param`. Повторный вызов с тем же параметром SHALL возвращать тот
же токен и не создавать новой записи в реестре токенов семейства. Токены семейства
SHALL быть полноценными `InjectionToken` — пригодными в deps
...
- **THEN** оба вызова возвращают токен с id `"Logger:users"` и токены
  идентичны (одна запись в реестре токенов семейства)
...
- **THEN** сборка успешна и токен семейства создан по рецепту
...
`build()` SHALL собирать все DI-токены зарегистрированных семейств, упомянутые в
...
графа. Несколько потребителей одного токена семейства SHALL получать один и тот же
инстанс (дедупликация). Рантайм-резолюции токенов семейства после `build()` SHALL NOT
существовать: токены семейства, не упомянутые в deps, не материализуются, и
...
- **WHEN** рецепт семейства A возвращает провайдер с dep-токеном семейства B
  (у B зарегистрирован свой рецепт)
- **THEN** токен семейства B материализуется на следующей итерации сбора, и сборка
...
параметра `p`, имеет `provide`, совпадающий с токеном токена семейства `family(p)`;
...
`build()` SHALL завершаться ошибкой, если токен семейства упомянут в deps
зарегистрированного провайдера, а `familyProvider` для этого семейства не
зарегистрирован; ошибка SHALL называть семейство и параметр DI-токена.
...
Токены семейства, ставшие узлами графа, SHALL участвовать во всех
механизмах контейнера наравне с обычными узлами: узлом токен семейства становится на `build()`,
значением — на INIT; детекция циклов (включая циклы через токены
...
#### Scenario: Захват и освобождение токена семейства происходят по одному разу

- **WHEN** рецепт возвращает `classProvider` с классом-ресурсом и вызваны
  `container.init()` и затем `container.destroy()`
- **THEN** `acquire` токена семейства вызван ровно один раз при `init()` и `release` —
  ровно один раз при `destroy()`

### Requirement: Family members inherit module attribution

Токены семейства SHALL получать `metadata.module` модуля, через `providers`
которого зарегистрирован их `familyProvider`, — так же, как обычные
провайдеры модуля. При регистрации
`familyProvider` напрямую в билдере (вне модуля) `metadata.module` токена семейства
...
- **WHEN** модуль `{ name: 'module:logging', providers: [familyProvider(ILogger, recipe)] }`
  зарегистрирован и токен семейства `ILogger('users')` материализован
...
- **THEN** узел материализованного токена семейства имеет `metadata.module === undefined`
