...
> плагин, операция», «[2026-09-03] Декларация приложения: `makeApp`,
> `assemble(select)`, `AssembledApp`» и «[2026-09-06] Переключатели
> состава: `makeSwitch`, `pick` и `when`, аргумент сборки; формы корня
> без фич».
...
`makeApp` объявляет приложение: список endpoint'ов и список транспортов.
`http()` объявляет HTTP-транспорт; порт и хост он берёт из своей секции
конфига, по умолчанию `3000` и `0.0.0.0`. Декларация — значение: она
ничего не читает и ничего не запускает. Проверяется она тоже при создании
— форма состава, перечень полей, — поэтому опечатка в словаре падает на
импорте, а не на старте.

Endpoint'ы стоят прямо в корне: единицы с именем сервису из одного
маршрута не нужны. Как приложение делится на части, когда областей
становится больше одной, показывает [2. Из чего состоит
приложение](./02-composition.md).
...
Итоговый пример уже разложен по фичам: у него две области, и endpoint'ы
...
Сервис из этой главы фич не объявляет, поэтому его строка короче:
...
Сервис отвечает на запрос без параметров. Дальше — одна схема всего, из
чего складывается приложение: [2. Из чего состоит
приложение](./02-composition.md).
