...
| 88 | `servers-implicit` | — | `server()` вместо `httpServer()`, имя по умолчанию `'default'`; `transports:` перечисляет только транспорты, серверы собираются транзитивно по ссылке `server` с дедупликацией; сервер без транспорта не создаётся | S | план — [ideas.md [2026-09-13]](./ideas.md) «Сервер: `server()`, не перечисляется в `transports:`»; независим |
