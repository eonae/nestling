...
Текущая фаза наблюдаема узлам ядра: узел проб читает её значением и
кладёт в отчёт готовности. DI-токена у фазы нет — прикладной код
полагается на фазовую модель, а не ветвится по фазе.
...
Пять модулей регистрируются всегда, и корень их не упоминает: конфиг
(слой конфигурации), асинхронный контекст запроса (слой пайплайна), порты
(слой портов), логгер (раздел «Логгер ядра») и пробы (раздел «Пробы»).
Узлы первых трёх — члены семейств DI-токенов, поэтому они создаются
только тогда, когда кто-то их инжектит; приложение, которое ими не
пользуется, не получает ни одного лишнего узла. Логгер и узел проб
регистрируются узлами всегда: сборка пишет через логгер сама, а
...
Число прочитанных элементов на это не влияет: `return()` до первого
элемента выполняет `.finally` так же, как закрытие на середине потока, и
доводит закрытие до источника, который вернул хендлер. Поэтому
`topic.subscribe()`, отданный непрочитанным ответом, снимается с темы, а
не висит до конца жизни процесса. Сколько элементов ушло клиенту, видно
в `ctx.summary.itemsOut`.
...
Ядро отдаёт состояние приложения значением: узел `Health$` собирает отчёт,
а формат и адрес выбирает тот, кто пробу обслуживает. HTTP-плагин
`httpProbes()` из [`@nestling/transport.http`](../nestling.transport.http)
переводит отчёт в коды 200 и 503.
...
export const DbModule = makeModule({
  name: 'module:db',
  providers: [classProvider(HealthCheck$('db'), DbHealthCheck)],
});
```

### Вклад: `HealthCheck$(name)`

Вклад — обычный провайдер члена семейства в том модуле, которому он
принадлежит; ничего кроме этой строки объявлять не нужно. Имя проверки
задаёт член, поэтому в самом вкладе имени нет. Вклады невыбранных фич
отсутствуют в отчёте тем же механизмом, который убирает из графа их
провайдеры; приложение без единого вклада собирается, а список проверок
пуст.

Интерфейс вклада — два члена: `critical: boolean` и
`check(signal): Promise<HealthStatus>`, где `HealthStatus` — `'ok'`,
`'degraded'` или `'down'`. Сигнал складывает отмену запроса и таймаут
проверки, поэтому вклад передаёт его дальше — в вызов зависимости.

Ресурс объявляет проверку короче: метод `health(signal)` у класса-ресурса
или одноимённое поле у `resourceProvider`
([`@nestling/container`](../nestling.container)). Сборка заводит вклад
сама, под идентификатором DI-токена ресурса и с `critical: true`.

### Узел: `Health$`

| Метод | Что делает |
|---|---|
| `liveness()` | синхронно возвращает `{ status: 'ok' }`; проверок не запускает |
| `readiness(signal?)` | собирает отчёт `{ status, phase, checks }` |

`liveness()` отвечает всегда: ответ и есть признак жизни процесса.
Вставший event loop не ответит в любом случае, а состояние зависимостей
перезапуском не чинится.

Итог `readiness` — `ready` тогда и только тогда, когда приложение в фазе
RUN и ни одна критичная проверка не вернула `down`. До RUN и на SHUTDOWN
итог `not_ready`, проверки не запускаются, список `checks` пуст: трафик
приложению в этих фазах не идёт. Статус `degraded` итога не меняет
никогда, `down` некритичной проверки — тоже.

Исход в отчёте — `{ name, critical, status, durationMs, reason? }`, где
`reason` — `'timeout'` или `'error'`. Сообщения ошибки и стека в отчёте
нет: тело пробы читает балансировщик и любой, кто дотянулся до порта.
Оригинал уходит в `Logger$('nestling:health')` записью `error` (критичная
проверка) или `warn` (некритичная) с полями `check`, `reason` и `err`.

Проверки одного вызова идут параллельно, каждая со своим таймаутом.
Исходы прогона кэшируются, а фаза и итог считаются заново на каждый вызов.
Вызовы, пришедшие во время незаконченного прогона, получают его результат,
поэтому две пробы подряд не превращаются в два запроса к базе.

### Секция `nestlingHealth`

| Ключ | Умолчание | Смысл |
|---|---|---|
| `NESTLING_HEALTH_TIMEOUT` | `2000` | предел одной проверки, мс |
| `NESTLING_HEALTH_CACHE` | `1000` | срок годности исходов прогона, мс |

Оба ключа — неотрицательные целые; значение вне диапазона роняет сборку с
именем ключа. `NESTLING_HEALTH_CACHE=0` отключает кэш: прогон идёт на
каждую пробу. Наружу пакет отдаёт `healthConfigKeys`, DI-токен секции
остаётся приватным.

### Справочник API: пробы

| Экспорт | Что это |
|---|---|
| `Health$` | DI-токен узла проб |
| `HealthCheck$(name)` | семейство вкладов; параметр члена — имя проверки |
| `Health`, `HealthCheck` | интерфейс узла и интерфейс вклада |
| `HealthStatus`, `ReadinessStatus` | исход проверки и итог готовности |
| `HealthReport`, `HealthCheckResult`, `LivenessReport` | формы отчётов |
| `healthConfigKeys` | ключи секции `nestlingHealth` для `config:` в корне |
| `healthKernel({ phase, checks })`, `resourceHealthChecks(resources)`, `registerHealth(builder, phase)` | точки, которыми пользуется composition root |
| `AppPhase` | текущая фаза приложения: `'INIT' \| 'WIRE' \| 'START' \| 'RUN' \| 'SHUTDOWN'` |

Не экспортируются: реализация узла и DI-токен секции `nestlingHealth`.
DI-токена текущей фазы не существует: узлы ядра получают её функцией
чтения.
