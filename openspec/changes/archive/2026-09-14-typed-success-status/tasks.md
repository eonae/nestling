...
- [x] 6.1 `src/response.ts`: `readSuccess` строит таблицу «код → форма
      ветки», валидирует тело схемой пришедшего исхода и отдаёт
      `InternalError`, когда код вне объявленного множества
- [x] 6.2 `src/client.ts`: объявленные исходы берутся с операции и попадают
      в разбор ответа; тип результата вызова — `Ok` или их юнион по
      объявлению
- [x] 6.3 Тесты `client.spec.ts` и `client.types.spec.ts`: разбор тела по
...
- [x] 9.2 Открытый вопрос дизайна (политика `check()` на недостижимую ветку
      ...
      возврата
- [x] 9.3 После `/opsx:archive`: строка 84 в `docs/decisions/roadmap.md` —
...
- [x] 10.7 Коммиты осмысленные, `main` не тронут — слияние делает Merger
      после `/opsx:archive`
