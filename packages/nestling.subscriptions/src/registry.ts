 ...
 * Поэтому `abort()` только взводит сигнал — снятие приедет обычным путём,
 * тем же, что и при дисконнекте клиента.
 */
...
export interface RegistryOptions {
   ...
   *
   * Факт наблюдения не вправе ронять подписку, поэтому ошибка `emit`
 ...
 *
 * Наружу не уезжает ни то ни другое — только снимок.
 */
interface Entry {
...
export class SubscriptionRegistry {
  ...
  open(ctx: SubscriptionContext): TrackedSubscription {
    ...
    // Лента — до вызова хендлера: собственного `opened` ручка живого
 ...
 * мутируется рантаймом, поэтому наружу уезжает число, а не ссылка.
 */
