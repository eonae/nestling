 ...
 * и классы-шаги пайплайна. `never` означает, что декларация готова к
...
> {
   ...
   * Пайплайн endpoint'а.
   *
   * Классы-шаги пайплайна попадают в `TNeeds` декларации и получают
   ...
   *
   * Резолвер создаёт инстанс класса-хендлера и инстансы классов-шагов
...
> {
  ...
  /**
   * Пайплайн endpoint'а. Классы-шаги допустимы: они попадают в `TNeeds`
  ...
  if (typeof detached !== 'string') {
    throw new TypeError(
      `${where}: 'detached' must be a non-empty string — the reason this ` +
        `handle is exempt from build policies. There is no ` +
        `'detached: true'.`,
    );
  }
 ...
 * есть на импорте модуля — до фазы BUILD и до первого запроса.
 *
 ...
 * `httpEndpoint.implement`): интерфейс операции принадлежит операции,
...
/**
 * Резолвит отложенные зависимости пайплайна — классы-шаги и DI-токены
...
function resolveDefinition(
  ...
  // при прошлом `resolve`. Остаётся создать инстансы классов-шагов
