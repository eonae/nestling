...
describe('overrides: подстановка узла графа', () => {
  ...
  it('объясняет отдельно, почему нет неинжектированного токена семейства', async () => {
...
describe('familyOverrides: подмена рецепта семейства', () => {
  it('создаёт всех токенов семейства тестовым рецептом, боевой не вызывается', async () => {
...
describe('прунинг: осиротевшие поддеревья', () => {
  ...
  it('оставляет токенов семейства, нужных только через `.all`', async () => {
    interface ISink {
      readonly scope: string;
    }
    ...
    const container = new ContainerBuilder({
      ...
      // Единственный, кто ссылается на токенов семейства по имени, — подменяемый узел
      .register(
        factoryProvider(Repository, () => ({ find: () => 'real' }), [
          ISinkFamily('a'),
          ISinkFamily('b'),
  ...
  it('собирает агрегат после прунинга — по оставшимся токенам семейства', async () => {
    interface ISink {
      readonly scope: string;
    }
    ...
    // Токен семейства 'b' нужен только подменяемому узлу и ни одному потребителю
    // `.all`, поэтому удаляется. Если бы агрегат создавался до прунинга,
    // его зависимости указывали бы на удалённого токена семейства, и сборка упала бы
    ...
    // Ребро на `.all` разворачивается в рёбра ко всем токенам семейства, поэтому
  ...
  it('выбрасывает токена семейства вместе с его единственным потребителем', async () => {
    interface ISink {
      readonly scope: string;
    }
...
describe('перечень недостающих зависимостей', () => {
  ...
  it('DI-токен, похожий на токена семейства, остаётся недостающей зависимостью', async () => {
    const ILogger = makeTokenFamily<{ scope: string }, [scope: string]>(
      'HintFamily',
    );
