/**
 * `endpoint-has-layer` — подсказка в редакторе: декларация ручки, похоже,
 * не композирована от требуемого слоя.
 *
 ...
 * причина, по которой правило молчит. Ложное срабатывание на легальном коде
 * хуже пропуска — пропущенный случай ловит policy-check.
 */
...
function callContainsLayer(
  ...
  if (callee.type === 'Identifier' && callee.name === COMPOSE) {
    ...
    // нечего: слой мог приехать именно в нём
  ...
  create(context) {
    ...
    return {
      CallExpression(node: CallExpression & Rule.NodeParentExtension) {
        ...
        // Spread делает словарь непрозрачным: `pipeline` мог приехать в нём
