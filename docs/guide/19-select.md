...
Маркер владеет процессом: вход, получивший `argv(process.argv)`,
завершает процесс сам. Справка уходит в `stdout` с кодом выхода `0`,
отказ — в `stderr` с кодом выхода `1`. Печатается сообщение отказа с
цепочкой причин под ним, а стека в выводе нет. Правило действует у всех
трёх входов, принимающих маркер, и покрывает отказ любой фазы подъёма.
Объектная форма процессом не владеет: `run()` отклоняет промис,
`discover()` и `check()` бросают, и отказ ловит вызывающий.
...
WIRE, где вызыватели связываются с владельцами операций:
...
другом процессе. Точка входа передала маркер, поэтому сообщение приходит
в `stderr` без стека, а процесс завершается кодом `1`.
