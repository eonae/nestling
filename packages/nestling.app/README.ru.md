...
конфигурации реализуют интерфейс `ConfigSource` отдельными пакетами —
Vault живёт в [`@nestlingjs/config.vault`](../nestling.config.vault/).
Порты не дедуплицируют команды по ключу идемпотентности, не хранят снапшоты
и не доставляют сообщения за пределы процесса: для этого нужен транспорт
брокера.
