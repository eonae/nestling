  ...
  return [
    ...
    },
    {
      ...
      rules: {
        ...
        /*
         * Форма условной примеси поля в объектный литерал: связка `&&` с
         * явным сравнением вместо тернарника с пустым литералом.
         *
         * Уровень `error`, а не `warn`: правило полно по форме, которую
         * разбирает, — пустой объектный литерал виден в тексте файла
         * целиком. Накопленное разобрано тем же change'ем, поэтому
         * переходная ступень не нужна.
         */
        '@nestlingjs/conditional-spread': 'error',
        /*
         * Список зависимостей декоратора роли против типов параметров
         * конструктора. Соглашение о DI-токенах общее для пакетов и
         * примеров (`conventions.md`), поэтому правило включено здесь.
         *
         * Уровень `warn` постоянный: правило синтаксическое и на
         * нестандартном DI-токене известного типа ошибается. Гарантия —
         * компилятор.
         */
    ...
    {
      /*
       * Швы с процессом. Окружение доходит до приложения источником
       * `env()`, командная строка — маркером `argv()`, и правило запрещает
       * третью дорогу.
       *
       * Область — `src` пакета, без тестового кода: тест ставит
       * переменные процесса, чтобы проверить чтение конфига, и это его
       * работа. Форма блока та же, что у языка строк ниже.
       *
       * Законное чтение глушится директивой с причиной в самом файле:
       * причина стоит рядом с кодом, а список путей в конфиге устаревает
       * молча.
       */
      files: ['src/**/*.ts'],
      ignores: [
