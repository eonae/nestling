...
| 57 | `kernel-boundary-outbox` | четыре места, в которые упёрся сателлит outbox'а: `Var.provide(compute)` без зависимостей из контейнера, событие без типизированного `meta.idempotencyKey`, текст ошибки о недостающей зависимости без точки расширения, раздел записи функцией плагина вместо аргумента `emit` | S–M | план — раздел «Результат замера границы» записи [ideas.md [2026-09-07]](./ideas.md) «Транзакционный outbox»; все четыре зафиксированы тестами `core-limits.spec.ts`; после 54 |
...
| 66 | `skill-setup` | скилл после первого внешнего прогона: спека собирает `app` из сниппетов через `assembleTest`, новые `references/setup.md` (tsconfig, скрипты, `tsx` для разработки, условие `testing`, конфиг `@nestlingjs/eslint-plugin`) и `references/http.md` (`redirect:`, `HttpResponse`, `Ok.created`), правила 6 и 7 в SKILL.md, таблица шести пакетов в «Where to look next», вариант `node:test` | S–M | реализован, ждёт архива — [ideas.md [2026-09-12]](./ideas.md) «Скилл после первого внешнего прогона» (помечена РЕАЛИЗОВАНО), [d/14](../history/discussions/14-first-external-run.md) пункты A1–A3, A5–A7, B8 |
| 67 | `feedback-polish` | пять починок по первому внешнему прогону: runtime-строки на английском во всех пакетах и проверка линтером, страховка от `any` в `ValidComponentShape`, JSDoc без `Ok.of`, `content-length: 0` у пустого ответа, `find-my-way` 9.7 и новее, раздел «когда Nestling не нужен» в корневом README | S | план — [ideas.md [2026-09-12]](./ideas.md) «Runtime-строки на английском и четыре починки», [d/14](../history/discussions/14-first-external-run.md) пункты B2, B4, B6, B7, B9, B10 |
| 68 | `http-endpoint-two-names` | форма с операцией получает своё имя `httpImplement(Operation, { … })`, у `httpEndpoint` остаётся анонимная форма: по две перегрузки на имя вместо четырёх, ложная ошибка на `pipeline:` исчезает; брендированный тип `UndeclaredFailError` в слоте `handler` называет незаявленный отказ; снапшоты диагностик, миграция примеров, глав и скилла, `references/diagnostics.md` в скилле | M, breaking | план — [ideas.md [2026-09-12]](./ideas.md) «Две декларации HTTP вместо четырёх перегрузок»; после 66 и 67; вопрос `httpEndpoint.get` из [deferred [2026-09-03]](./deferred.md) решается на propose |
| 69 | `openapi-query-parsed-form` | параметр path и query описывается разобранной формой схемы, если она скалярная: второй прогон конвертера с `io: 'output'` в `planParameters`, `z.stringbool()` даёт `type: boolean`; тело запроса без изменений | S–M | план — [ideas.md [2026-09-12]](./ideas.md) «Документ OpenAPI: параметр в разобранной форме», [d/14](../history/discussions/14-first-external-run.md) пункт B5 |
...
| 57 | `kernel-boundary-outbox` | S–M | замер границы ядра нашёл четыре места и намеренно не чинил их: правка ядра ради сателлита обесценила бы сам замер, но сами места остались |
...
| 66 | `skill-setup` | S–M | скилл давал код, который не проходил ASSEMBLE: сниппеты компилируются по одному и как приложение противоречат друг другу; настройки проекта и HTTP-формы ответа в скилле не было, и агент искал их опытом |
| 67 | `feedback-polish` | S | пять мест, о которые первый внешний пользователь споткнулся за час: русские строки в OpenAPI-документе, чужой текст в ошибке `@Component`, JSDoc про несуществующий `Ok.of`, редирект с `transfer-encoding: chunked`, `npm audit` с тремя high по `find-my-way` 8.x |
| 68 | `http-endpoint-two-names` | M, breaking | цена четырёх перегрузок, принятая записью [2026-08-01], оказалась на стороне пользователя: ошибка незаявленного отказа не называет отказ и добавляет ложную строку на `pipeline:`; форм хендлера осталось две, и две перегрузки на имя стоят меньше прежних шести |
| 69 | `openapi-query-parsed-form` | S–M | канон `z.stringbool()` для query из записи [2026-08-01] документировался как `type: string`, и генератор клиента делал из булева поля строку |
