...
единственное место, где читается `process.env`. Значения складываются в
снимок: обычный объект без обращений к сети. Если источник временно
недоступен, делается ограниченное число повторов, потом процесс
завершается. Результат фазы — снимок конфига.
...
топологическом порядке закрывает пулы и соединения.
...
атрибутируются единице с именем приложения. Сквозная инфраструктура
...
источников ([config.md §3](./config.md)). Подмена узлов графа
...
|---|---|---|
| L0 | endpoint'ы, провайдеры и транспорт в корне | фич, `select`, конфиг-секций, операций, интеркома |
| L1 | типизированные конфиг-секции | фич, `select`, операций, интеркома |
| L2 | фичи, `select` и переключатели состава | операций, интеркома; все фичи в одном процессе |
...
упоминает ни фич, ни `select`, ни операций, ни интеркома: за
...
### L2 — фичи, `select` и переключатели
...
- Переключатель стоит в любом списке единиц: `providers:`, `modules:`
  ...
  корня. В `features:` его нет: состав фич выбирает `select`. В
...
- DI-токена у переключателя нет: инжектировать выбор нельзя, состав не
  ...
  переключатель, визуализация показывает невыбранные ветки серым, строка
  старта печатает выбор рядом с фичами:
...
списками `features:` и `plugins:`, переключателями и `select`, а не
...
процессам. Отсюда и место в корне: `plugins:` не участвует в `select`.
...
так работают контейнер и `select`. «Есть в процессе» означает только,
...
([container.md](./container.md), там же интерфейс). Ядро регистрирует
`ConsoleLogger` по умолчанию; провайдер приложения под тем же DI-токеном
заменяет умолчание без ошибки дубля. Уровень задаёт kernel-секция
...
`Logger$('nestling')` и его дочерние области: строку состава на старте —
`info`, предупреждения конфига — `warn`, незадекларированный отказ на
границе — `error` с оригиналом в `err`. Отдельных хуков вывода у ядра
нет. Адаптеры к pino и подобным — satellite-пакеты.
