...
|---|---|---|---|
| `run(options?)` | `AssembledApp` | 0–5 | доводит приложение до RUN и остаётся там; ставит обработчики сигналов. `options.config` — привязки источников вместо умолчания ([config.md §3](./config.md)) |
