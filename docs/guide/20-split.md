...
как в главе [19](./19-select.md). Останавливает на фазе BUILD, поэтому
тот же отказ видят `check()` и матрица топологий — до запуска процесса.
...
Тест поднимает оба процесса в одном процессе раннера поверх двойника брокера
