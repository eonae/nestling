...
подключает `@nestling/outbox`, который на ней работает.
...
[главы 8](./08-logging.md): значение приходит из контекста запроса, а не
...
[главы 12](./12-features.md). Хранилище outbox'а инжектит фоновая задача
...
Пакет `@nestling/outbox` подключается плагином и получает три вещи:
...
Предикат тот же `hasVar` из [главы 9](./09-auth.md), только переменную
...
([глава 7](./07-testing.md)), поэтому проход делает сам тест:
...
([глава 17](./17-split.md)): записи создают все процессы, а разбирает их
