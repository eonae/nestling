...
после того, как поток закончился или оборвался, поэтому исход честен:
отключение клиента даёт `disconnected`, источник, который закончился сам,
даёт `completed`, отказ даёт `failed`. Отказ посреди потока приходит
